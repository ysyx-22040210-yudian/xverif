...
- [kverif-sdk: 二次开发接口](#kverif-sdk-二次开发接口)
...
flowchart LR
    ...
    Developer["二次开发脚本"] --> SDK["kverif-sdk"]
...
kverif/
  ...
  kverif_sdk/            # 面向验证二次开发的公共 Python SDK、workflow 和示例
...
| `kverif_sdk/` | 公共 Python transport/client/workflow、二次开发示例和 mock 测试 | 二次开发者常用 |
...
| [`kverif-sdk`](kverif_sdk/README.md) | 验证脚本二次开发 | Python API、FSDB/daidir/VDB | 波形、连线、coverage workflow report | mock 不需要；真实查询取决于后端 |
...
`kverif_sdk` 是面向验证团队二次开发的公共 Python 层。它使用稳定 JSON 协议，
...
| 波形查询 client | `KdebugClient` | FSDB 批量取值、变化查询、driver/graph 追踪 |
| coverage client | `KcovClient` | VDB summary、holes 和 session 生命周期管理 |
...
```text
kverif.sdk.wave-window.v1
kverif.sdk.module-connections.v1
kverif.sdk.coverage-convergence.v1
```

workflow 会保留每次底层工具的原始 response。项目脚本可以在标准汇总字段上增加
自己的 checker，同时仍能访问新增 NPI 证据字段。SDK 只把
`kverif_sdk.__init__` 导出的对象作为公共 API，项目代码不应导入 kdebug、kcov
或 MCP 的内部实现。
...
先以 VM 普通用户 `host` 准备环境。`PYTHON` 必须导出，因为 `tools/kcov`
子进程也要使用 Python 3.8+：
...
$PYTHON -m kverif_sdk.examples.coverage_convergence \
  --tool /home/host/kverif/tools/kcov \
  --run real=/home/host/testdata/kcov_no_timeout_smoke_20260714/simv.vdb \
  --metrics line,toggle \
  --hole-limit 20 \
...
[`kverif_sdk/README.md`](kverif_sdk/README.md)。
...
### Python 找不到 `kverif_sdk` 或 `kverif_mcp`
...
- [`kverif_sdk/README.md`](kverif_sdk/README.md): Python 二次开发 SDK、workflow 和示例
