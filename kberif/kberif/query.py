...
from .errors import CARDS_CATALOG_INVALID, DETAIL_REQUIRED_MISSING, KIND_MISMATCH, TOPIC_NOT_FOUND, KberifError
...
def status(root: Path) -> dict:
    ...
    if not cfg_path.exists() or not topics_path.exists():
        ...
        if catalog_count > 0:
            ...
            state = "configured_only"
    return {
        "schema_version": "kberif.status.v1",
        "state": state,
...
def _next_action_for_state(state: str) -> str:
    if state == "not_configured":
        return "run kberif config init --kind <bt|it|st|soc>"
    if state == "configured_only":
        return "run kberif init --model <model>"
    if state == "generated_raw":
        return "run kberif repair-catalog"
    if state == "invalid":
        return "run kberif validate and fix reported cards/details"
    return "use kberif brief/get/detail"


def _catalog_or_error(root: Path) -> dict:
    catalog_path = state_dir(root) / "cards.json"
    if not catalog_path.exists():
        raise KberifError(CARDS_CATALOG_INVALID, ".kberif/cards.json is missing; run kberif status")
    ...
    if not catalog.get("cards"):
        ...
        if st["state"] == "generated_raw":
            raise KberifError(
                ...
                ".kberif/cards.json is empty but raw cards/details exist; run kberif repair-catalog",
            )
        raise KberifError(CARDS_CATALOG_INVALID, ".kberif/cards.json has no cards; run kberif status")
    return catalog
...
def get_topic(root: Path, topic: str) -> dict:
    ...
    return {
        "schema_version": "kberif.topic_result.v1",
        "env_kind": kind,
        "topic": topic,
        "source_card": card["id"],
        "title": card["title"],
        ...
        "detail_method": "kberif.get_topic_detail",
...
def get_topic_detail(root: Path, topic: str) -> dict:
    ...
    if not path.exists():
        raise KberifError(DETAIL_REQUIRED_MISSING, f"topic {topic} has no detail")
    content = read_text(path)
    stats = validate_detail(root, topic, content, path=path)
    return {
        "schema_version": "kberif.topic_detail_result.v1",
        "env_kind": env_kind(root),
        "topic": topic,
        "card_id": cfg["card_id"],
        "format": "markdown",
...
def brief_result(root: Path, mode: str) -> dict:
    ...
    if view.get("env_kind") != kind:
        raise KberifError(KIND_MISMATCH, f"view {mode} env_kind does not match {kind}")
    topics = []
        ...
        topics.append(
            {
                ...
                "detail_hint": f"Use kberif.get_topic_detail(topic='{topic}') for deeper analysis.",
            }
        )
    return {"schema_version": "kberif.brief.v1", "env_kind": kind, "mode": mode, "topics": topics}


def brief(root: Path, mode: str) -> str:
    parts = [f"# kberif brief: {mode}", ""]
    for result in brief_result(root, mode)["topics"]:
        ...
        if result["detail_available"]:
            parts.extend(["", f"Detail: `kberif detail {result['topic']}`"])
        parts.append("")
    return "\n".join(parts).rstrip() + "\n"
