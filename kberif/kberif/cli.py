...
from .kout import render_list_topics, render_repair, render_status, render_topic
...
def _print_query_json_or_kout(data: dict, renderer) -> None:
    if OUTPUT_JSON:
...
def validate_cmd(all_: bool = typer.Option(False, "--all")) -> None:
    ...
    def work():
        ...
        if errors:
            ...
            raise KberifError(VALIDATION_FAILED, "validation failed")
        console.print("ok")
...
def status_cmd() -> None:
    ...
    _run(lambda: _print_query_json_or_kout(status(Path.cwd()), render_status))
...
def repair_catalog_cmd() -> None:
    ...
    def work():
        ...
        _print_query_json_or_kout({"ok": True, "catalog_card_count": len(catalog.get("cards", []))}, render_repair)
...
def list_topics_cmd() -> None:
    ...
    _run(lambda: _print_query_json_or_kout(list_topics(Path.cwd()), render_list_topics))
...
def get_cmd(topic: str, detail: bool = typer.Option(False, "--detail")) -> None:
    ...
    if detail:
        ...
        _run(lambda: _print_query_json_or_kout(get_topic(Path.cwd(), topic), render_topic))
...
def card_append_key_items(card_id: str, stdin: bool = typer.Option(False, "--stdin")) -> None:
    ...
    def work():
        ...
        if not isinstance(key_items, list):
            raise KberifError("CARD_SCHEMA_INVALID", "stdin must be a JSON list")
...
def _shortcut(namespace: str, topic: str) -> None:
    ...
    def work():
        ...
        _print_query_json_or_kout(get_topic(root, topic.replace("-", "_")), render_topic)
...
def bootstrap_state() -> None:
    ...
    def work():
        root = Path.cwd()
        cfg = read_toml(root / "kberif" / "kind.toml")
        kind = cfg["env_kind"]
        write_json(state_dir(root) / "kind.json", {"schema_version": "kberif.kind_state.v1", "env_kind": kind})
