...
def empty_catalog(kind: str) -> dict:
    return {"schema_version": "kberif.cards_catalog.v1", "env_kind": kind, "cards": []}
...
def _manifest(root: Path) -> dict:
    ...
    if not path.exists():
        raise KberifError(MANIFEST_MISSING, ".kberif/manifest.json is missing")
    ...
    for ev in evidence:
        ...
        if isinstance(ev, str):
            ...
            if match:
                ...
                continue
        raise KberifError(CARD_SCHEMA_INVALID, "evidence must be objects or path:start-end strings")
    return normalized
...
def normalize_card_evidence(card: dict[str, Any]) -> bool:
    ...
    for item in card.get("key_items", []):
        ...
        if not isinstance(evidence, list):
            raise KberifError(CARD_SCHEMA_INVALID, "key_item evidence must be a list")
...
def _validate_evidence(root: Path, evidence: list[Any]) -> None:
    ...
    for ev in evidence:
        ...
        if not path or path not in files:
            raise KberifError(EVIDENCE_PATH_NOT_IN_MANIFEST, f"evidence path {path} is not in manifest")
        if _matches(path, deny_paths):
            raise KberifError(DENY_PATH_REFERENCED, f"evidence path {path} matches deny_paths")
        ...
        if not isinstance(start, int) or not isinstance(end, int) or start < 1 or end < start or end > max_lines:
            raise KberifError(EVIDENCE_LINE_RANGE_INVALID, f"invalid evidence line range for {path}")


def _expected_detail_ref(card_id: str) -> str:
    return f".kberif/details/{card_id}.md"


def validate_card(root: Path, card: dict[str, Any]) -> None:
    kind = load_kind_config(root)["env_kind"]
    if card.get("schema_version") != "kberif.topic_card.v1":
        raise KberifError(CARD_SCHEMA_INVALID, "card schema_version must be kberif.topic_card.v1")
    if card.get("env_kind") != kind:
        raise KberifError(KIND_MISMATCH, f"card env_kind {card.get('env_kind')} does not match {kind}")
    topic = card.get("topic")
    cfg = topic_def(root, topic)
    if card.get("id") != cfg.get("card_id"):
        raise KberifError(CARD_ID_MISMATCH, f"card id must be {cfg.get('card_id')}")
    if card.get("title") != cfg.get("title"):
        raise KberifError(CARD_SCHEMA_INVALID, f"card title must be {cfg.get('title')}")
    if not isinstance(card.get("summary"), str) or not card["summary"].strip():
        raise KberifError(CARD_SCHEMA_INVALID, "card summary must contain a short, clear topic conclusion")
    key_items = card.get("key_items")
    if not isinstance(key_items, list):
        raise KberifError(CARD_SCHEMA_INVALID, "card key_items must be a list")
    if cfg.get("required") and not key_items:
        raise KberifError(CARD_SCHEMA_INVALID, "required card must contain key_items")
    if len(key_items) > 8:
        raise KberifError(CARD_SCHEMA_INVALID, "card key_items must contain no more than 8 entries")
    normalize_card_evidence(card)
    for item in key_items:
        if not isinstance(item, dict) or not item.get("name") or not item.get("one_line"):
            raise KberifError(CARD_SCHEMA_INVALID, "each key_item requires name and one_line")
        _validate_evidence(root, item.get("evidence", []))
    detail = card.get("detail")
    if not isinstance(detail, dict) or detail.get("available") is not True:
        raise KberifError(CARD_SCHEMA_INVALID, "card detail.available must be true")
    if detail.get("path") != _expected_detail_ref(card["id"]) or detail.get("format") != "markdown":
        raise KberifError(CARD_SCHEMA_INVALID, "card detail reference is invalid")
...
def validate_card_file(root: Path, path: Path) -> None:
    ...
    if resolved.parent != expected_dir:
        raise KberifError(CARD_SCHEMA_INVALID, f"card file must be under {expected_dir}")
    card = read_json(resolved)
    validate_card(root, card)
    if resolved.name != f"{card['id']}.json":
        raise KberifError(CARD_ID_MISMATCH, f"card filename must be {card['id']}.json")


def _frontmatter(content: str) -> tuple[dict[str, str], str]:
    if not content.startswith("---\n") or "\n---\n" not in content[4:]:
        raise KberifError(DETAIL_SCHEMA_INVALID, "detail must start with YAML-style frontmatter")
    raw, body = content[4:].split("\n---\n", 1)
    meta: dict[str, str] = {}
    for line in raw.splitlines():
        if ":" not in line:
            raise KberifError(DETAIL_SCHEMA_INVALID, "detail frontmatter must use key: value lines")
...
def validate_detail(root: Path, topic: str, content: str, path: Path | None = None) -> dict[str, Any]:
    ...
    if path is not None and path.resolve() != detail_path(root, card_id).resolve():
        raise KberifError(DETAIL_CARD_MISMATCH, f"detail filename must be {card_id}.md")
    ...
    for key, value in expected.items():
        if meta.get(key) != value:
            raise KberifError(DETAIL_CARD_MISMATCH, f"detail {key} must be {value}")
    if not meta.get("confidence"):
        raise KberifError(DETAIL_SCHEMA_INVALID, "detail confidence is required")
    for section in DETAIL_SECTIONS:
        if not re.search(rf"^##\s+{re.escape(section)}\s*$", body, re.MULTILINE):
            raise KberifError(DETAIL_SECTION_MISSING, f"detail section is missing: {section}")
    evidence = [
        {"path": match.group(1), "line_start": int(match.group(2)), "line_end": int(match.group(3))}
        for match in EVIDENCE_RE.finditer(body)
    ]
    if not evidence:
        raise KberifError(DETAIL_SCHEMA_INVALID, "detail Evidence section must contain parseable evidence")
...
def validate_detail_file(root: Path, path: Path) -> dict[str, Any]:
    ...
    if resolved.parent != expected_dir or resolved.suffix != ".md":
        raise KberifError(DETAIL_SCHEMA_INVALID, f"detail file must be under {expected_dir}")
...
def update_catalog(root: Path) -> dict:
    ...
    catalog = {"schema_version": "kberif.cards_catalog.v1", "env_kind": kind, "cards": cards}
...
def append_key_items(root: Path, card_id: str, key_items: list[dict[str, Any]]) -> None:
    ...
    if not path.exists():
        raise KberifError(CARD_SCHEMA_INVALID, f"card {card_id} must be upserted before appending key_items")
    card = read_json(path)
...
def validate_all(root: Path) -> list[str]:
    ...
    if not skind_path.exists():
        errors.append(f"{KIND_MISMATCH}: .kberif/kind.json is missing")
    elif read_json(skind_path).get("env_kind") != kind:
        errors.append(f"{KIND_MISMATCH}: .kberif/kind.json does not match kberif/kind.toml")
    try:
        _manifest(root)
    except KberifError as exc:
        errors.append(f"{exc.code}: {exc.message}")
    catalog_path = state_dir(root) / "cards.json"
    if not catalog_path.exists():
        errors.append(f"{CARDS_CATALOG_INVALID}: .kberif/cards.json is missing")
        catalog = {"cards": []}
    else:
    ...
    for topic, cfg in read_toml(config_dir(root) / "topics.toml").get("topics", {}).items():
        ...
        except (OSError, ValueError, KberifError) as exc:
            if isinstance(exc, KberifError):
                errors.append(f"{exc.code}: {exc.message}")
            else:
