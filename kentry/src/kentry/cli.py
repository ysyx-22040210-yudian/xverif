...
from .format import dumps, error_response, to_kout
from .fragments import load_fragments_file


HELP_TEXT = """kentry - multi-beat entry field decoder

Usage:
  kentry -h
  kentry -help
  kentry decode --config entry.yaml --input fragments.jsonl [--json]
  kentry explain --config entry.yaml [--json]
  kentry validate --config entry.yaml [--input fragments.jsonl]
  kentry -
  kentry request.json
  kentry '{"api_version":"kentry.v1","action":"decode",...}'
...
def emit(payload: dict, *, pretty: bool, json_mode: bool = False) -> int:
    print(dumps(payload, pretty=pretty) if json_mode else to_kout(payload), end="" if not json_mode else "\n")
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="kentry", description="deterministic entry field decoder")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if "--text" in argv or "--kout" in argv:
        argv = [arg for arg in argv if arg not in {"--text", "--kout"}]
