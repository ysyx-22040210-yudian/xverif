...
from kentry.config import load_config_text, normalize_config
from kentry.decode import decode_entry
from kentry.errors import ConfigError, FragmentError, UnsupportedConfigField
from kentry.fragments import bytes_to_bits, parse_hex_bytes
...
class CliTests(unittest.TestCase):
    def run_cli(self, *args, input_text=None):
        ...
        proc = subprocess.run(
            [sys.executable, "-m", "kentry.cli", *args],
            cwd=str(ROOT),
            env=env,
            input=input_text,
            check=True,
    ...
    def test_json_stdin(self):
        request = {"api_version": "kentry.v1", "action": "decode", "config": CONFIG, "fragments": [
    ...
    def test_json_arg(self):
        request = {"api_version": "kentry.v1", "action": "explain", "config": CONFIG}
        payload = self.run_cli("--json", json.dumps(request))
        self.assertTrue(payload["ok"])
        self.assertEqual(payload["action"], "explain")

    def test_json_file(self):
        request = {"api_version": "kentry.v1", "action": "validate", "config": CONFIG}
    ...
    def test_kout_default(self):
        request = {"api_version": "kentry.v1", "action": "explain", "config": CONFIG}
        env = os.environ.copy()
        env["PYTHONPATH"] = str(SRC)
        proc = subprocess.run(
            [sys.executable, "-m", "kentry.cli", json.dumps(request)],
            cwd=str(ROOT),
            env=env,
            check=True,
            text=True,
        ...
        self.assertTrue(proc.stdout.startswith("@kentry.explain.v1"))
