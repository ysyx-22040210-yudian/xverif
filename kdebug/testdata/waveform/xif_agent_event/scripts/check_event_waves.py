...
def query(binary, home, action, args=None, target=None, expect_ok=True):
    request = {"api_version": "kdebug.v1", "action": action, "args": args or {},
...
def main():
    ...
    parser.add_argument("--kdebug", default=str(REPO_ROOT / "tools" / "kdebug"))
    ...
    home = Path(tempfile.mkdtemp(prefix="kdebug-event-check-"))
    session = "xif_event_check"
    try:
        query(args.kdebug, home, "session.open",
        ...
        for name, config in configs.items():
            query(args.kdebug, home, "event.config.load",
                  {"name": name, "config_path": str(ROOT / config)},
                  {"session_id": session})

        def export(name, expr):
            return query(args.kdebug, home, "event.export",
                         {"name": name, "expr": expr},
        ...
        query(args.kdebug, home, "event.find",
              {"name": "rdy", "expr": "vld && missing_alias"},
              {"session_id": session}, expect_ok=False)
        print("PASS: kdebug event checks rdy={} bp={} none={} pair_master={} pair_slave={}".format(
            len(rdy), len(bp), len(none), len(pair_master), len(pair_slave)))
    finally:
        try:
            query(args.kdebug, home, "session.kill", {"id": "all"})
        except Exception:
            pass
