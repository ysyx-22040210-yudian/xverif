...
本文是 kdebug / kverif 日志系统改造的执行依据。目标是在不污染
...
- `kdebug/src/core/logging/action_log.cpp` 已提供 action、lifecycle、transport 三类
  NDJSON 事件接口。
...
```json
{
  ...
  "component": "kverif-mcp|kdebug|engine|waveform|kcov",
  "session_id": "case0",
  "alias": "case0",
  "action": "value.at",
...
- 默认路径：`~/.kdebug/sessions/adhoc/logs/stdio.ndjson`。
- 能识别 session 后，同步写 `~/.kdebug/sessions/<session_id>/logs/stdio.ndjson`。
- 事件包括：
  - `loop.ready`
  - `loop.invalid_json`
...
- `KDEBUG_LOG_MAX_BYTES`：单个日志文件大小上限。
- `KDEBUG_LOG_MAX_FILES`：保留滚动文件数量。
- 达到上限时按 `.1`, `.2` 滚动。
...
- `KdebugLoopSession.open/query/abort/close`。
...
- `KDEBUG_LOG_PATH_MODE=full|basename|hash`，默认 `full`。
- `KDEBUG_LOG_REDACT=0|1`，默认 `0`。
- `KVERIF_MCP_LOG_DIR`，默认 `~/.kverif/mcp`。

后续新增 CLI：

- `kdebug log tail --session <id>`
- `kdebug log doctor --session <id>`
...
扩展 `kdebug/tests/unit/test_action_log.cpp`：
...
扩展 `kdebug/tests/session/test_stdio_loop_lifecycle.py`：
...
扩展 `kverif_mcp/tests/test_stdio_loop_session_lifecycle.py`：
...
- `kdebug/README.md`：日志路径、排障顺序、`log-test`。
- `skill/SKILL.md`：agent 使用 kverif 工具和日志排障的最短路径。
- `skill/references/kdebug/troubleshooting.md`：kdebug 原生日志路径和 bundle/doctor/tail 说明。
- `skill/references/mcp/troubleshooting.md`、`skill/references/sdk-free-kdebug/troubleshooting.md`：
  MCP / SDK-free wrapper 日志路径和 ready timeout、stdout pollution、job id 排障方式。
- `kverif_mcp/README.md`：MCP / LSF 日志路径、ready timeout、stdout pollution、job id
  排障方式。

## 实施顺序

1. 先落地阶段一和对应 targeted tests。
2. 跑 `make -C kdebug log-test`，再跑 `unit-test` / `contract-test`。
