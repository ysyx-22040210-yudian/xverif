#!/usr/bin/env python3
"""Validate kdebug JSON examples against local schema files.

The validator implements the small JSON Schema subset used by kdebug contract
...
def validate_file(path: Path, schemas: Path) -> None:
    ...
    if path.parent.name == "errors":
        schema_path = schemas / "kdebug.error.schema.json"
    else:
...
def main(argv: List[str]) -> int:
    examples = Path(argv[1]) if len(argv) > 1 else Path("kdebug/examples")
    schemas = Path(argv[2]) if len(argv) > 2 else Path("kdebug/schemas/v1")
