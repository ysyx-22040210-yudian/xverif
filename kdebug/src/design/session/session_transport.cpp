...
bool write_endpoint_file(const SessionInfo& session) {
    ...
    return kdebug_core::atomic_write_json_file(kdebug_design_endpoint_path(session.session_id), root);
}

bool read_endpoint_file(const std::string& session_id, SessionInfo& endpoint) {
    FILE* fp = fopen(kdebug_design_endpoint_path(session_id).c_str(), "r");
    if (!fp) return false;
    std::string text;
    char buf[1024];
    ...
    try {
        ...
        endpoint.socket_path = e.value("socket_path", kdebug_design_socket_path(session_id));
...
int connect_session_endpoint(const SessionInfo& session) {
    ...
    if (is_tcp_transport(session)) {
        return kdebug_core::connect_tcp(session.host, session.port);
    }
    return kdebug_core::connect_uds(
        session.socket_path.empty() ? kdebug_design_socket_path(session.session_id) : session.socket_path);
}
...
bool send_file_request_to_endpoint(const SessionInfo& session, const Json& request, Json& response, int timeout_ms) {
    ...
    std::string dir = session.file_dir.empty()
        ? kdebug_core::file_transport_dir(kdebug_design_session_dir(session.session_id))
        : session.file_dir;
...
static bool request_simple(const SessionInfo& session, const std::string& action, Json& data) {
    if (is_file_transport(session)) {
        ...
        Json response;
        if (!send_file_request_to_endpoint(session, request, response, kdebug_core::file_transport_ping_timeout_ms()))
            return false;
    ...
    std::string line;
    if (ok) {
        ok = kdebug_core::read_line_timeout(fd, line);
        if (ok) {
            try {
