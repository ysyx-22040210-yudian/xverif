...
bool kdebug_design_debug_enabled() {
    const char* env = getenv("KDEBUG_DESIGN_DEBUG");
...
void SessionManager::debug_log(const char* fmt, ...) const {
    if (!kdebug_design_debug_enabled()) return;
    fprintf(stderr, "[kdebug_design-debug] ");
    va_list ap;
    va_start(ap, fmt);
...
pid_t SessionManager::spawn_server(const std::string& session_id, const std::vector<std::string>& args,
    ...
    if (pid == 0) {
        ...
        // survives after `kdebug_design open ...` exits.
        if (setsid() < 0) {
            _exit(1);
        }
...
bool SessionManager::dbdir_metadata_matches(const SessionInfo& expected, const SessionInfo& current) const {
    return kdebug_core::resource_content_matches(expected.dbdir_mtime,
                                                 expected.dbdir_size,
                                                 current.dbdir_mtime,
                                                 current.dbdir_size);
}
...
WaitForServerResult SessionManager::wait_for_server(const std::string& session_id, pid_t pid) {
    ...
    int timeout_sec = 60;
    const char* env = getenv("KDEBUG_DESIGN_SESSION_START_TIMEOUT_SEC");
    if (env && *env) {
        int v = atoi(env);
    ...
    kdebug_core::log_lifecycle_event("design", session_id, "wait_for_server.begin", true,
    ...
    for (int i = 0; i < loops; ++i) {
        ...
        if (!endpoint_ready) {
            ...
            endpoint.socket_path = kdebug_design_socket_path(session_id);
        ...
        if (endpoint_ready) {
            if (is_file_transport(endpoint)) {
                ...
                if (result.ping_ok) {
                    ...
                    kdebug_core::log_lifecycle_event("design", session_id, "wait_for_server.ready", true,
        ...
        if (waitpid(pid, &status, WNOHANG) > 0) {
            ...
            kdebug_core::log_lifecycle_event("design", session_id, "wait_for_server.child_exited", false,
    ...
    debug_log("wait_for_server: timeout reason=%s elapsed_ms=%ld socket_exists=%d connect_ok=%d ping_ok=%d",
              ...
              result.connect_ok ? 1 : 0, result.ping_ok ? 1 : 0);
    kdebug_core::log_lifecycle_event("design", session_id, "wait_for_server.timeout", false,
...
SessionEnsureResult SessionManager::ensure_session(const std::vector<std::string>& design_args,
    ...
    if (!parse_open_args(design_args, canonical_dbdir, canonical_fsdb, canonical_args)) {
        ...
        kdebug_core::log_lifecycle_event("design", session_name, "ensure_session.invalid_args", false);
    ...
    kdebug_core::log_lifecycle_event("design", session_name, "ensure_session.canonicalized", true,
    ...
    if (!SessionRegistry::is_valid_session_name(session_name)) {
        ...
        kdebug_core::log_lifecycle_event("design", session_name, "ensure_session.invalid_session_id", false);
        return result;
    }
    ...
    if (!valid_transport(requested_transport)) {
        ...
        kdebug_core::log_lifecycle_event("design", session_name, "ensure_session.invalid_transport", false,
    ...
    kdebug_core::log_lifecycle_event("design", session_name, "cleanup.begin", true);
    cleanup();
    debug_log("ensure_session: cleanup_stale_done");
    kdebug_core::log_lifecycle_event("design", session_name, "cleanup.end", true);
    ...
    if (registry_->exists(session_name)) {
        ...
        kdebug_core::log_lifecycle_event("design", session_name, "ensure_session.session_id_exists", false);
        return result;
    }

    std::string session_id = session_name;
    if (!kdebug_design_ensure_session_dir(session_id)) {
        ...
        kdebug_core::log_lifecycle_event("design", session_id, "ensure_session.session_dir_failed", false);
        return result;
    }
    debug_log("ensure_session: session_id=%s", session_id.c_str());
    kdebug_core::log_lifecycle_event("design", session_id, "ensure_session.session_dir_ready", true,
                                     {{"session_dir", kdebug_design_session_dir(session_id)}});

    SessionInfo endpoint;
    endpoint.session_id = session_id;
    endpoint.transport = requested_transport;
    endpoint.socket_path = kdebug_design_socket_path(session_id);
    endpoint.file_dir = kdebug_core::file_transport_dir(kdebug_design_session_dir(session_id));
    ...
    if (pid < 0) {
        ...
        result.message = "Failed to spawn kdebug_design server";
        kdebug_design_remove_session_dir(session_id);
        debug_log("ensure_session: reason=spawn_failed session=%s", session_id.c_str());
        kdebug_core::log_lifecycle_event("design", session_id, "ensure_session.spawn_failed", false);
        return result;
    }
    debug_log("ensure_session: spawned_server session=%s pid=%d", session_id.c_str(), pid);
    kdebug_core::log_lifecycle_event("design", session_id, "ensure_session.spawned_server", true,
    ...
    if (!wait.ok) {
        ...
        kdebug_core::log_lifecycle_event("design", session_id, "ensure_session.startup_failed", false,
    ...
    if (!registry_->add(session)) {
        ...
        kdebug_core::log_lifecycle_event("design", session_id, "ensure_session.registry_failed", false);
        return result;
    }
    ...
    kdebug_core::log_lifecycle_event("design", session_id, "ensure_session.success", true,
...
bool SessionManager::kill_session(const std::string& session_id) {
    ...
    if (!registry_->get(session_id, session)) {
        ...
        kdebug_core::log_lifecycle_event("design", session_id, "kill_session.registry_missing", false);
        return false;
    }
    ...
    kdebug_core::log_lifecycle_event("design", session_id, "kill_session.begin", true,
    ...
    if (is_local_session_host(session)) {
        ...
        if (!stopped) {
            if (!process_matches_session(session)) {
                ...
                kdebug_core::log_lifecycle_event("design", session_id,
    ...
    kdebug_core::log_lifecycle_event("design", session_id, "kill_session.end", true,
                                     {{"pid", session.server_pid}, {"quit_sent", quit_sent}});
    return true;
}
...
bool SessionManager::kill_all_sessions() {
    ...
    kdebug_core::log_lifecycle_event("design", "adhoc", "kill_all.begin", true,
                                     {{"count", sessions.size()}});
    bool all_ok = true;
    for (const auto& session : sessions) {
        if (!kill_session(session.session_id)) all_ok = false;
    }
    kdebug_core::log_lifecycle_event("design", "adhoc", "kill_all.end", all_ok);
    return all_ok;
}
...
SessionHealth SessionManager::diagnose_session(const std::string& session_id) {
    ...
    if (!registry_->get(session_id, session)) {
        ...
        kdebug_core::log_lifecycle_event("design", session_id, "diagnose.registry_missing", false);
        return health;
    }
    ...
    if (!session.dbdir_path.empty()) {
        if (!current_dbdir_metadata(session, current)) {
            ...
            kdebug_core::log_lifecycle_event("design", session_id, "diagnose.dbdir_missing", false,
                                             {{"dbdir", session.dbdir_path}});
            return health;
        }
        if (!dbdir_metadata_matches(session, current)) {
            bool identity_changed = kdebug_core::resource_identity_differs(session.dbdir_dev,
                                                                           session.dbdir_inode,
                                                                           current.dbdir_dev,
                                                                           current.dbdir_inode);
            health.status = SessionHealthStatus::DbdirChanged;
            health.message = "Daidir metadata changed since session was opened";
            kdebug_core::log_lifecycle_event("design", session_id, "diagnose.dbdir_changed", false,
                                             {{"dbdir", session.dbdir_path},
        ...
        if (kdebug_core::resource_identity_differs(session.dbdir_dev,
                                                   session.dbdir_inode,
                                                   current.dbdir_dev,
                                                   current.dbdir_inode)) {
            ...
            kdebug_core::log_lifecycle_event("design", session_id, "diagnose.dbdir_identity_changed", true,
                                             {{"dbdir", session.dbdir_path},
                                              {"old_dev", session.dbdir_dev}, {"new_dev", current.dbdir_dev},
                                              {"old_inode", session.dbdir_inode}, {"new_inode", current.dbdir_inode},
    ...
    if (!session.fsdb_file.empty()) {
        if (!current_fsdb_metadata(session, current)) {
            ...
            kdebug_core::log_lifecycle_event("design", session_id, "diagnose.fsdb_missing", false,
        ...
        if (!fsdb_metadata_matches(session, current)) {
            ...
            kdebug_core::log_lifecycle_event("design", session_id, "diagnose.fsdb_changed", false,
                                             {{"fsdb", session.fsdb_file},
    ...
    if (is_local_session_host(session) && kill(session.server_pid, 0) != 0) {
        ...
        kdebug_core::log_lifecycle_event("design", session_id, "diagnose.process_exited", false,
                                         {{"pid", session.server_pid}});
        return health;
    }
    ...
    if (!is_tcp_transport(session) && !is_file_transport(session) && access(session.socket_path.c_str(), F_OK) != 0) {
        ...
        kdebug_core::log_lifecycle_event("design", session_id, "diagnose.socket_missing", false,
    ...
    if (fd < 0) {
        ...
        kdebug_core::log_lifecycle_event("design", session_id, "diagnose.connect_failed", false,
    ...
    if (!ping_session_endpoint(session)) {
        ...
        kdebug_core::log_lifecycle_event("design", session_id, "diagnose.ping_failed", false,
                                         {{"socket_path", session.socket_path}});
        return health;
    }
    ...
    kdebug_core::log_lifecycle_event("design", session_id, "diagnose.healthy", true,
...
void SessionManager::cleanup() {
    ...
    kdebug_core::log_lifecycle_event("design", "adhoc", "cleanup.begin", true);
    registry_->cleanup_stale();
    debug_log("cleanup: done");
    kdebug_core::log_lifecycle_event("design", "adhoc", "cleanup.end", true);
}
