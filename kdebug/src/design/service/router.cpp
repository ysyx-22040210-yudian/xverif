...
json schema_payload() {
    return {{"api_version", API_VERSION}, {"request", {{"api_version", API_VERSION}, {"request_id", "optional-id"},
        ...
        {"response", {{"ok", true}, {"action", "trace.driver"}, {"tool", {{"name", "kdebug_design"}, {"version", TOOL_VERSION}}},
        ...
        {"transport", {{"default", "uds"}, {"env_default", "KDEBUG_TRANSPORT"}, {"values", json::array({"uds", "tcp", "file"})},
...
json handle_request_impl(const json& request) {
    ...
    if (api_ver != API_VERSION && api_ver != "kdebug.v1") {
        return error_response(request, action, "UNSUPPORTED_API_VERSION",
            "expected kdebug.internal.v1 or kdebug.v1", false);
    }
...
json handle_request(const json& request) {
    ...
    kdebug_core::log_action_event("backend", "design", sid, action, "begin", true, 0,
    ...
    kdebug_core::log_action_event("backend", "design", sid, action, "end", ok, elapsed_ms,
                                  {{"response", kdebug_core::response_summary_for_log(response)},
                                   {"request", ok ? kdebug_core::request_summary_for_log(request) : kdebug_core::sanitize_for_log(request)}});
    return response;
}
