...
Json make_kbit_hints(const Json& args, const std::string& signal, const std::string& raw) {
    Json hint_args = args.value("slice_hint", Json::object());
    bool include = bool_or(args, "include_kbit_hints", false) ||
    ...
    if (chunk_width <= 0) {
        return {{"status", "needs_slice_hint"},
                {"message", "Provide args.slice_hint.chunk_width to generate deterministic kbit slice commands"},
    ...
    for (long long i = 0; i < count; ++i) {
        ...
        commands.push_back("tools/kbit slice \"" + value + "\" " + std::to_string(hi) + " " + std::to_string(lo) + " --json");
    }
...
Json make_value_payload(const Json& args, const std::string& signal, const std::string& raw, bool compact) {
    ...
    Json hints = make_kbit_hints(args, signal, raw);
    if (!hints.is_null()) payload["kbit_hints"] = hints;
    return payload;
}
...
public:
    ...
    int run(const WaveformActionContext& ctx) const override {
        ...
        if (payload.contains("kbit_hints")) out["data"]["kbit_hints"] = payload["kbit_hints"];
        return emit(ctx, out);
    }
};
    ...
    int run(const WaveformActionContext& ctx) const override {
        ...
        for (const auto& s : ctx.args["signals"]) {
            ...
            if (query_value(ctx.sid, signal, time, fmt_char(ctx.args), raw, err)) {
                ...
                if (payload.contains("kbit_hints")) item["kbit_hints"] = payload["kbit_hints"];
                if (item["status"] == "ok") {
                    LogicValue value = logic_value_from_fsdb_raw(raw, fmt_char(ctx.args));
