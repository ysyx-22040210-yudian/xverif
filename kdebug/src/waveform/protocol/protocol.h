...
#define SOCK_PATH_PREFIX    ".kdebug_waveform"
...
inline void get_sock_path(char* buf, const std::string& session_id) {
    snprintf(buf, SOCK_PATH_LEN, "%s", kdebug_waveform::kdebug_waveform_socket_path(session_id).c_str());
}
...
inline void get_registry_path(char* buf) {
    snprintf(buf, SOCK_PATH_LEN, "%s", kdebug_waveform::kdebug_waveform_registry_path().c_str());
}

// Get registry lock file path
inline void get_registry_lock_path(char* buf) {
    snprintf(buf, SOCK_PATH_LEN, "%s", kdebug_waveform::kdebug_waveform_registry_lock_path().c_str());
}

// Get server-side debug log path for a session
inline void get_debug_log_path(char* buf, const std::string& session_id) {
    snprintf(buf, SOCK_PATH_LEN, "%s", kdebug_waveform::kdebug_waveform_debug_log_path(session_id).c_str());
}
