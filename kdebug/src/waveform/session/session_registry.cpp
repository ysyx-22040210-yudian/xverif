...
SessionRegistry::SessionRegistry() {
    ...
    registry_path_ = kdebug_waveform_registry_path();
}
    ...
    try {
        ...
        for (const auto& item : root["sessions"]) {
            ...
            if (json_to_session(item, session)) {
                ...
                if (session.socket_path.empty()) session.socket_path = kdebug_waveform_socket_path(session.session_id);
...
bool SessionRegistry::write_session_file(const SessionInfo& session) {
    ...
    std::string path = kdebug_waveform_session_json_path(session.session_id);
...
bool SessionRegistry::cleanup_stale() {
    ...
    for (const auto& session : sessions) {
        ...
        bool endpoint_exists = (session.transport == "tcp" || session.transport == "file")
            ? (access(kdebug_waveform_endpoint_path(session.session_id).c_str(), F_OK) == 0)
            : (access(session.socket_path.c_str(), F_OK) == 0);
        if (is_alive && endpoint_exists) {
            valid_sessions.push_back(session);
        } else {
            kdebug_waveform_remove_session_dir(session.session_id);
        }
    }
