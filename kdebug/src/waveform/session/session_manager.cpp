...
std::string default_transport_from_env() {
    const char* env = getenv("KDEBUG_TRANSPORT");
...
void SessionManager::debug_log(const char* fmt, ...) const {
    ...
    fprintf(stderr, "[kdebug_waveform-debug] ");
    va_list ap;
    va_start(ap, fmt);
...
bool SessionManager::fsdb_metadata_matches(const SessionInfo& expected, const SessionInfo& current) const {
    return kdebug_core::resource_content_matches(expected.fsdb_mtime,
                                                 expected.fsdb_size,
                                                 current.fsdb_mtime,
                                                 current.fsdb_size);
}
...
WaitForServerResult SessionManager::wait_for_server(const std::string& session_id, pid_t pid) {
    ...
    kdebug_core::log_lifecycle_event("waveform", session_id, "wait_for_server.begin", true,
    ...
    for (int i = 0; i < iterations; ++i) {
        ...
        if (!endpoint_ready) {
            ...
            endpoint.socket_path = kdebug_waveform_socket_path(session_id);
        ...
        if (endpoint_ready) {
            if (is_file_transport(endpoint)) {
                ...
                if (result.ping_ok) {
                    ...
                    kdebug_core::log_lifecycle_event("waveform", session_id, "wait_for_server.ready", true,
        ...
        if (waitpid(pid, &status, WNOHANG) > 0) {
            ...
            kdebug_core::log_lifecycle_event("waveform", session_id, "wait_for_server.child_exited", false,
    ...
    debug_log("wait_for_server: timeout reason=%s elapsed_ms=%ld child_alive=%d endpoint_exists=%d connect_ok=%d ping_ok=%d",
              ...
              result.ping_ok ? 1 : 0);
    kdebug_core::log_lifecycle_event("waveform", session_id, "wait_for_server.timeout", false,
    ...
    if (kill(pid, 0) == 0 && result.reason == "timeout_waiting_endpoint") {
        ...
        debug_log("wait_for_server: server is still alive; it may still be opening FSDB. Increase KDEBUG_WAVEFORM_SESSION_START_TIMEOUT_SEC or inspect %s", log_path);
    }
    return result;
}
...
pid_t SessionManager::spawn_server(const std::string& session_id, const std::string& fsdb_file,
    ...
    if (pid == 0) {
        ...
        // CLI invocations such as `kdebug_waveform open ...`.
        if (setsid() < 0) {
            _exit(1);
        }
...
std::string SessionManager::create_session(const std::string& fsdb_file,
    ...
    if (!SessionRegistry::is_valid_session_name(session_id)) {
        ...
        kdebug_core::log_lifecycle_event("waveform", session_id, "create_session.invalid_session_id", false);
        return "";
    }
    ...
    if (!valid_transport(requested_transport)) {
        ...
        kdebug_core::log_lifecycle_event("waveform", session_id, "create_session.invalid_transport", false,
    ...
    kdebug_core::log_lifecycle_event("waveform", session_id, "create_session.canonicalized", true,
    ...
    if (!populate_fsdb_metadata(canonical, metadata)) {
        ...
        kdebug_core::log_lifecycle_event("waveform", session_id, "create_session.fsdb_stat_failed", false,
    ...
    if (lock_fd < 0) {
        ...
        kdebug_core::log_lifecycle_event("waveform", session_id, "create_session.registry_lock_open_failed", false,
    ...
    if (flock(lock_fd, LOCK_EX) != 0) {
        ...
        kdebug_core::log_lifecycle_event("waveform", session_id, "create_session.registry_lock_failed", false,
    ...
    if (registry_->exists(session_id)) {
        ...
        kdebug_core::log_lifecycle_event("waveform", session_id, "create_session.session_id_exists", false);
        flock(lock_fd, LOCK_UN);
        close(lock_fd);
        return "";
    }
    if (!kdebug_waveform_ensure_session_dir(session_id)) {
        debug_log("create_session: reason=session_dir_create_failed session=%s", session_id.c_str());
        kdebug_core::log_lifecycle_event("waveform", session_id, "create_session.session_dir_failed", false);
        flock(lock_fd, LOCK_UN);
        close(lock_fd);
        return "";
    }
    ...
    endpoint.socket_path = kdebug_waveform_socket_path(session_id);
    endpoint.file_dir = kdebug_core::file_transport_dir(kdebug_waveform_session_dir(session_id));
    ...
    if (pid < 0) {
        ...
        kdebug_core::log_lifecycle_event("waveform", session_id, "create_session.spawn_failed", false,
                                         {{"errno", errno}, {"message", strerror(errno)}});
        kdebug_waveform_remove_session_dir(session_id);
        flock(lock_fd, LOCK_UN);
        close(lock_fd);
        return "";
    }
    ...
    kdebug_core::log_lifecycle_event("waveform", session_id, "create_session.spawned_server", true,
    ...
    if (!wait.ok) {
        ...
        kdebug_core::log_lifecycle_event("waveform", session_id, "create_session.startup_failed", false,
    ...
    if (!registry_->add(session)) {
        ...
        kdebug_core::log_lifecycle_event("waveform", session_id, "create_session.registry_add_failed", false);
        kill(pid, SIGTERM);
        unlink(sock_path);
        kdebug_waveform_remove_session_dir(session_id);
        flock(lock_fd, LOCK_UN);
    ...
    if (!health.healthy) {
        ...
        kdebug_core::log_lifecycle_event("waveform", session_id, "create_session.post_create_health_failed", false,
    ...
    kdebug_core::log_lifecycle_event("waveform", session_id, "create_session.success", true,
...
bool SessionManager::stop_process(const SessionInfo& session, bool remove_record, bool remove_events) {
    kdebug_core::log_lifecycle_event("waveform", session.session_id, "stop_process.begin", true,
    ...
    if (is_local_session_host(session)) {
        ...
        if (kill(session.server_pid, 0) == 0) {
            kdebug_core::log_lifecycle_event("waveform", session.session_id, "stop_process.sigterm", true,
                                             {{"pid", session.server_pid}});
            kill(session.server_pid, SIGTERM);
            usleep(300000);
        }
        if (kill(session.server_pid, 0) == 0) {
            kdebug_core::log_lifecycle_event("waveform", session.session_id, "stop_process.sigkill", true,
                                             {{"pid", session.server_pid}});
    ...
    if (!remove_record && remove_events) kdebug_waveform_remove_session_dir(session.session_id);
    kdebug_core::log_lifecycle_event("waveform", session.session_id, "stop_process.end", true);
    return true;
}
...
bool SessionManager::restart_session(const std::string& session_id) {
    ...
    kdebug_core::log_lifecycle_event("waveform", session_id, "restart_session.begin", true);
    int lock_fd = open_registry_lock();
    if (lock_fd < 0) {
        debug_log("restart_session: reason=registry_lock_open_failed errno=%d(%s)",
                  errno, strerror(errno));
        kdebug_core::log_lifecycle_event("waveform", session_id, "restart_session.registry_lock_open_failed", false,
                                         {{"errno", errno}, {"message", strerror(errno)}});
        return false;
    }
    if (flock(lock_fd, LOCK_EX) != 0) {
        debug_log("restart_session: reason=registry_lock_failed errno=%d(%s)",
                  errno, strerror(errno));
        kdebug_core::log_lifecycle_event("waveform", session_id, "restart_session.registry_lock_failed", false,
                                         {{"errno", errno}, {"message", strerror(errno)}});
        close(lock_fd);
        return false;
    }
    ...
    if (!registry_->get(session_id, old_session)) {
        ...
        kdebug_core::log_lifecycle_event("waveform", session_id, "restart_session.registry_missing", false);
        flock(lock_fd, LOCK_UN);
        close(lock_fd);
        return false;
    }
    ...
    if (!current_fsdb_metadata(old_session, metadata)) {
        ...
        kdebug_core::log_lifecycle_event("waveform", session_id, "restart_session.fsdb_stat_failed", false,
    ...
    if (!is_local_session_host(old_session)) {
        ...
        kdebug_core::log_lifecycle_event("waveform", session_id, "restart_session.remote_restart_required", false,
    ...
    if (pid < 0) {
        debug_log("restart_session: reason=spawn_failed errno=%d(%s)",
                  errno, strerror(errno));
        kdebug_core::log_lifecycle_event("waveform", session_id, "restart_session.spawn_failed", false,
                                         {{"errno", errno}, {"message", strerror(errno)}});
        flock(lock_fd, LOCK_UN);
        close(lock_fd);
        return false;
    ...
    kdebug_core::log_lifecycle_event("waveform", session_id, "restart_session.spawned_server", true,
    ...
    if (!wait.ok) {
        debug_log("restart_session: reason=%s elapsed_ms=%ld child_exited=%d child_status=%d socket_exists=%d connect_ok=%d ping_ok=%d",
                  ...
                  wait.child_status, wait.socket_exists ? 1 : 0,
                  wait.connect_ok ? 1 : 0, wait.ping_ok ? 1 : 0);
        kdebug_core::log_lifecycle_event("waveform", session_id, "restart_session.startup_failed", false,
                                         {{"reason", wait.reason}, {"elapsed_ms", wait.elapsed_ms},
                                          {"child_exited", wait.child_exited}, {"child_status", wait.child_status},
                                          {"socket_exists", wait.socket_exists}, {"connect_ok", wait.connect_ok},
                                          {"ping_ok", wait.ping_ok}});
    ...
    kdebug_core::log_lifecycle_event("waveform", session_id, "restart_session.registry_upsert", ok,
                                     {{"pid", static_cast<int>(pid)}});
    flock(lock_fd, LOCK_UN);
    close(lock_fd);
    return ok;
...
bool SessionManager::ensure_session_current(const std::string& session_id) {
    ...
    kdebug_core::log_lifecycle_event("waveform", session_id, "ensure_session_current.begin", true);
    SessionInfo session;
    if (!registry_->get(session_id, session)) {
        debug_log("ensure_session_current: reason=registry_missing session=%s", session_id.c_str());
        kdebug_core::log_lifecycle_event("waveform", session_id, "ensure_session_current.registry_missing", false);
        return false;
    }
    ...
    if (!current_fsdb_metadata(session, current)) {
        ...
        kdebug_core::log_lifecycle_event("waveform", session_id, "ensure_session_current.fsdb_stat_failed", false,
                                         {{"fsdb", session.fsdb_file}, {"errno", errno}, {"message", strerror(errno)}});
        return false;
    }
    if (!fsdb_metadata_matches(session, current)) {
        bool identity_changed = kdebug_core::resource_identity_differs(session.fsdb_dev,
                                                                       session.fsdb_inode,
                                                                       current.fsdb_dev,
                                                                       current.fsdb_inode);
        ...
        kdebug_core::log_lifecycle_event("waveform", session_id, "ensure_session_current.fsdb_changed_restart", true,
    ...
    if (kdebug_core::resource_identity_differs(session.fsdb_dev,
                                               session.fsdb_inode,
                                               current.fsdb_dev,
                                               current.fsdb_inode)) {
        debug_log("ensure_session_current: fsdb_identity_changed session=%s old_dev=%llu new_dev=%llu old_inode=%llu new_inode=%llu content_match=1",
                  ...
                  session.fsdb_inode, current.fsdb_inode);
        kdebug_core::log_lifecycle_event("waveform", session_id, "ensure_session_current.fsdb_identity_changed", true,
                                         {{"old_dev", session.fsdb_dev}, {"new_dev", current.fsdb_dev},
                                          {"old_inode", session.fsdb_inode}, {"new_inode", current.fsdb_inode},
    ...
    kdebug_core::log_lifecycle_event("waveform", session_id, "ensure_session_current.diagnose", health.healthy,
                                     {{"status", session_health_status_name(health.status)}, {"message", health.message}});
...
bool SessionManager::kill_session(const std::string& session_id) {
    ...
    kdebug_core::log_lifecycle_event("waveform", session_id, "kill_session.begin", true);
    SessionInfo session;
    if (!registry_->get(session_id, session)) {
        debug_log("kill_session: reason=registry_missing session=%s", session_id.c_str());
        kdebug_core::log_lifecycle_event("waveform", session_id, "kill_session.registry_missing", false);
        return false;
    }
    ...
    if (!health.healthy) {
        if (is_local_session_host(session) && kill(session.server_pid, 0) == 0) {
            ...
            kdebug_core::log_lifecycle_event("waveform", session_id, "kill_session.stale_sigterm", true,
                                             {{"pid", session.server_pid}});
            kill(session.server_pid, SIGTERM);
            usleep(300000);
        ...
        registry_->remove(session_id);
        kdebug_core::log_lifecycle_event("waveform", session_id, "kill_session.removed_unhealthy", true,
                                         {{"status", session_health_status_name(health.status)}, {"message", health.message}});
        return true;
    }
...
bool SessionManager::kill_all_sessions() {
    ...
    kdebug_core::log_lifecycle_event("waveform", "adhoc", "kill_all.begin", true,
                                     {{"count", sessions.size()}});
    for (const auto& session : sessions) {
        kill_session(session.session_id);
    }
    registry_->clear_all();
    kdebug_core::log_lifecycle_event("waveform", "adhoc", "kill_all.end", true);
    return true;
}
...
bool SessionManager::gc_sessions() {
    ...
    kdebug_core::log_lifecycle_event("waveform", "adhoc", "gc.begin", true);
    cleanup();
    const char* env_timeout = getenv("KDEBUG_WAVEFORM_IDLE_TIMEOUT_SEC");
    int timeout = env_timeout ? atoi(env_timeout) : 1800;
    if (timeout <= 0) timeout = 1800;
    debug_log("gc_sessions: idle_timeout_sec=%d", timeout);
    kdebug_core::log_lifecycle_event("waveform", "adhoc", "gc.timeout", true,
    ...
    for (const auto& session : sessions) {
        ...
        if (last > 0 && now - last > timeout) {
            debug_log("gc_sessions: removing_idle session=%s pid=%d idle_sec=%ld timeout_sec=%d",
                      ...
                      timeout);
            kdebug_core::log_lifecycle_event("waveform", session.session_id, "gc.removing_idle", true,
                                             {{"pid", session.server_pid},
    ...
    kdebug_core::log_lifecycle_event("waveform", "adhoc", "gc.end", true);
    return true;
}
...
SessionHealth SessionManager::diagnose_session(const std::string& session_id) {
    ...
    if (!registry_->get(session_id, session)) {
        ...
        debug_log("diagnose_session: status=%s message=%s",
                  ...
                  health.message.c_str());
        kdebug_core::log_lifecycle_event("waveform", session_id, "diagnose.registry_missing", false);
        return health;
    }
    ...
    if (!current_fsdb_metadata(session, current)) {
        ...
        debug_log("diagnose_session: status=%s message=%s",
                  session_health_status_name(health.status),
                  health.message.c_str());
        kdebug_core::log_lifecycle_event("waveform", session_id, "diagnose.fsdb_missing", false,
    ...
    if (!fsdb_metadata_matches(session, current)) {
        ...
        debug_log("diagnose_session: status=%s message=%s",
                  session_health_status_name(health.status),
                  health.message.c_str());
        kdebug_core::log_lifecycle_event("waveform", session_id, "diagnose.fsdb_changed", false,
                                         {{"fsdb", session.fsdb_file},
                                          {"old_mtime", session.fsdb_mtime}, {"new_mtime", current.fsdb_mtime},
                                          {"old_size", session.fsdb_size}, {"new_size", current.fsdb_size},
                                          {"old_dev", session.fsdb_dev}, {"new_dev", current.fsdb_dev},
    ...
    if (kdebug_core::resource_identity_differs(session.fsdb_dev,
        ...
        debug_log("diagnose_session: fsdb_identity_changed old_dev=%llu new_dev=%llu old_inode=%llu new_inode=%llu content_match=1",
                  ...
                  (unsigned long long)session.fsdb_inode,
                  (unsigned long long)current.fsdb_inode);
        kdebug_core::log_lifecycle_event("waveform", session_id, "diagnose.fsdb_identity_changed", true,
                                         {{"fsdb", session.fsdb_file},
                                          {"old_dev", session.fsdb_dev}, {"new_dev", current.fsdb_dev},
                                          {"old_inode", session.fsdb_inode}, {"new_inode", current.fsdb_inode},
                                          {"content_match", true}, {"identity_changed", true}});
    ...
    if (is_local_session_host(session) && kill(session.server_pid, 0) != 0) {
        ...
        debug_log("diagnose_session: status=%s pid=%d errno=%d(%s)",
                  ...
                  errno,
                  strerror(errno));
        kdebug_core::log_lifecycle_event("waveform", session_id, "diagnose.process_exited", false,
    ...
    if (!is_tcp_transport(session) && !is_file_transport(session) && access(session.socket_path.c_str(), F_OK) != 0) {
        ...
        debug_log("diagnose_session: status=%s socket=%s errno=%d(%s)",
                  ...
                  errno,
                  strerror(errno));
        kdebug_core::log_lifecycle_event("waveform", session_id, "diagnose.socket_missing", false,
    ...
    if (fd < 0) {
        ...
        debug_log("diagnose_session: status=%s socket=%s",
                  session_health_status_name(health.status),
                  session.socket_path.c_str());
        kdebug_core::log_lifecycle_event("waveform", session_id, "diagnose.connect_failed", false,
                                         {{"transport", session.transport}, {"socket_path", session.socket_path},
                                          {"host", session.host}, {"port", session.port}});
        return health;
    }
    ...
    if (!ping_session_endpoint(session)) {
        ...
        debug_log("diagnose_session: status=%s socket=%s",
                  session_health_status_name(health.status),
                  session.socket_path.c_str());
        kdebug_core::log_lifecycle_event("waveform", session_id, "diagnose.ping_failed", false,
    ...
    debug_log("diagnose_session: status=%s message=%s",
              session_health_status_name(health.status),
                  health.message.c_str());
    kdebug_core::log_lifecycle_event("waveform", session_id, "diagnose.healthy", true,
...
void SessionManager::cleanup() {
    ...
    kdebug_core::log_lifecycle_event("waveform", "adhoc", "cleanup.begin", true,
                                     {{"before_count", before.size()}});
    registry_->cleanup_stale();
    std::vector<SessionInfo> after;
    registry_->load_all(after);
    kdebug_core::log_lifecycle_event("waveform", "adhoc", "cleanup.end", true,
