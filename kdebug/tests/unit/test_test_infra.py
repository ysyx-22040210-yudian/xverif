...
def _fake_kdebug(tmp_path: Path) -> Path:
    script = tmp_path / "fake_kdebug"
    script.write_text(
        """#!/usr/bin/env python3
import json
import os
...
if "--stdio-loop" in sys.argv:
    print(json.dumps({"type": "ready", "protocol": "kdebug-stdio-loop",
    ...
    for line in sys.stdin:
        ...
        if req.get("output", {}).get("format") == "json" or "--json" in sys.argv:
            ...
            envelope = {"id": rid, "ok": True, "payload_format": "kout",
                        "kout": "@kdebug.fake.v1\\nsummary:\\n  value: 7\\n"}
...
def test_cli_runner_stdin_and_file(tmp_path: Path) -> None:
    runner = CliRunner(_fake_kdebug(tmp_path), cwd=tmp_path)
    request = {"api_version": "kdebug.v1", "action": "actions"}
...
@pytest.mark.unit
def test_cli_runner_kout(tmp_path: Path) -> None:
    runner = CliRunner(_fake_kdebug(tmp_path), cwd=tmp_path)
    result = runner.run(
    ...
    assert result.ok
    assert result.response.startswith("@kdebug.fake.v1")


@pytest.mark.unit
def test_stdio_loop_json_and_quit(tmp_path: Path) -> None:
    loop = StdioLoopRunner(_fake_kdebug(tmp_path), cwd=tmp_path)
    ready = loop.start()
    assert ready["protocol"] == "kdebug-stdio-loop"
    result = loop.request(
        {
...
def test_runners_record_history_for_failure_artifacts(tmp_path: Path) -> None:
    cli = CliRunner(_fake_kdebug(tmp_path), cwd=tmp_path)
    cli_result = cli.run({"api_version": "kdebug.v1", "action": "actions"})
    ...
    loop.start()
    try:
        loop_result = loop.request({"api_version": "kdebug.v1", "action": "actions"})
