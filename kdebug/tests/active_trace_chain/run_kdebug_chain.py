...
def usage():
    print(
        "usage: run_kdebug_chain.py KDEBUG SESSION DAIDIR FSDB SIGNAL TIME OUT_DIR",
        file=sys.stderr,
    )
...
def extract_json(text):
    start = text.find("{")
    if start < 0:
        raise ValueError("no JSON object in kdebug output")
    depth = 0
    in_string = False
    escaped = False
    ...
    raise ValueError("unterminated JSON object in kdebug output")


def run_kdebug(kdebug, request, timeout_sec=180):
    proc = subprocess.run(
        [kdebug, "--json", "-"],
...
def main(argv):
    if len(argv) != 7:
        usage()
        return 2
    kdebug, session, daidir, fsdb, signal, time_spec, out_dir = argv
    ...
    open_resp = run_kdebug(kdebug, open_req)
    ...
    try:
        ...
        chain_resp = run_kdebug(kdebug, chain_req)
        ...
        try:
            run_kdebug(kdebug, close_req, timeout_sec=30)
        except Exception:
            pass
    return 0
