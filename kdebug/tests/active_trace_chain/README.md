...
验证 kdebug 是否可以基于 **多次 native active trace** 构造完整因果链。
...
common/                 # 共享脚本
  run_kdebug_chain.py   # 核心：反复调用 trace.active_driver
...
# 2. 打开 kdebug session
kdebug --json - <<'JSON'
{"api_version":"kdebug.v1","action":"session.open",
 "target":{"daidir":"out/simv.daidir","fsdb":"out/waves.fsdb"},
 "args":{"name":"p0_0","reopen":true}}
JSON

# 3. 运行链
python3 ../common/run_kdebug_chain.py p0_0 "top.out" "10ns" ../actual/p0_0_assign_chain
