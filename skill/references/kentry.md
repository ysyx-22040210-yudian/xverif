...
kentry 把多拍 byte fragments 按 config 切成 raw field slices。不要让 LLM 自己跨拍拼接或 hex slicing。
...
优先 MCP `kverif_entry_*`。命令行：

```bash
tools/kentry --json '{"api_version":"kentry.v1","action":"decode","config":{},"fragments":[]}'
printf '%s\n' '<json-request>' | tools/kentry --json -
```
...
- kentry 只输出 raw，不做 enum/IP/MAC/协议语义解释。
