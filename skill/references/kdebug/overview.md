...
kdebug 是 kverif 的 daidir/FSDB 事实查询入口，用 JSON request 查询设计、波形和 combined 因果证据。设计事实来自 `daidir`，波形事实来自 `fsdb`，两者同时存在时可做 active-driver、协议异常和源码 evidence join。
...
不要用 kdebug 做 bit 计算、entry decode、日志位置还原或项目 context 管理；分别用 kbit、kentry、kloc、kberif。

## 入口

优先使用 MCP 或 SDK-free wrapper。MCP 场景下，本文所有 kdebug 原生 action 从 `kverif_debug_query` 进入：

1. 用 `kverif_debug_session_open` 打开 design/waveform/combined session。
2. 用 `kverif_debug_query` 传 `action`、`args`、`limits`、`output`。
3. 常规 debug 不默认用 `kverif_debug_raw_request`；raw request 不维护 MCP 托管的 stdio-loop/LSF session。
...
机器解析必须请求 JSON。默认 `kout` 适合人读或 AI 摘要，不适合脚本字段比较。
