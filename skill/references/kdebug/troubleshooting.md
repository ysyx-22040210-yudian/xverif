...
- public actions：`~/.kdebug/sessions/<session_prefix>_<hash>/logs/actions.ndjson`
- stdio-loop：同目录 `stdio.ndjson`
- engine lifecycle/transport/crash：`~/.kdebug/engine/sessions/<hashed-session>/logs/`
...
可用 `KDEBUG_LOG_PATH_MODE=basename|hash` 或 `KDEBUG_LOG_REDACT=1` 控制路径字段。
