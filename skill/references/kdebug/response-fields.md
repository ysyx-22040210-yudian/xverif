...
本文是 `kdebug.v1` JSON API 的响应字段手册，覆盖当前 action catalog 中的所有命令。MCP 的 `kverif_debug_query` 在 `output_format:"json"` 时返回这些 backend JSON 字段；`output_format:"kout"` 返回适合 AI 阅读的文本摘要，`output_format:"envelope"` 用于定位 wrapper/stdio-loop 问题。它面向 AI agent 和脚本作者，目标是说明每个 action 成功响应里可能出现的 `summary`、`data`、`findings`、`meta`、`error` 字段，以及 compact/full/debug 下字段是否会被省略。

机器可校验契约以 action-specific response schema 为准：`kdebug/schemas/v1/actions/<action>.response.schema.json`。本文负责解释字段含义；schema 和 `kdebug/examples/responses/<action>.basic.json` 负责约束 compact 主路径。
...
所有 action 最终都归一化到 `kdebug.v1` envelope。

| 字段 | 类型 | 出现条件 | 含义 |
| --- | --- | --- | --- |
| `api_version` | string | 总是 | 固定为 `kdebug.v1` |
| `request_id` | any | 请求带 `request_id` 时 | 调用方透传 ID |
| `ok` | boolean | 总是 | action 是否成功 |
| `action` | string | 总是 | 实际响应 action |
| `tool.name` | string | full/debug 常见 | 工具名，归一化为 `kdebug` |
...
`transport/host/bind_host/port/socket_path` 通常只在 `session.open/list/doctor`、`output.verbosity:"full"` / `"debug"` 或后端日志中出现。compact action 响应不会反复携带 endpoint 信息；连接问题优先读 `~/.kdebug/{design,waveform}/sessions/<hashed-session>/logs/transport.ndjson`。

`fingerprint.dev` / `fingerprint.inode` 是路径所在挂载位置的 identity 诊断字段，不是内容 freshness contract。跨登录机、计算节点或容器访问同一共享路径时它们可能不同；kdebug 判定资源是否变化只依赖 `mtime + size`。日志里的 `identity_changed:true` 表示位置身份变化，不等同于 FSDB/daidir 内容变化。
...
| `data.results[]` | array | 每个 child 的完整 kdebug response |
...
| `data.session.host` / `data.session.port` | string/number | TCP endpoint；只在 TCP/full/debug/后端响应中稳定有意义 |
| `data.session.file_dir` | string | file transport endpoint；只在 file/full/debug/后端响应中稳定有意义 |
...
| 路径 | 类型 | 含义 |
| --- | --- | --- |
| `summary.session_count` | number | 顶层 kdebug session 数 |
...
| `data.values[].kbit_hints` | object | 传 `slice_hint` 时生成的 kbit 命令 |
