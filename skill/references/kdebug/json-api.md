...
本文说明 kdebug 原生 `kdebug.v1` action 的 request、target、output、transport 和示例。MCP、SDK-free wrapper、raw CLI 都复用这套 action 契约。

## MCP query 入口规则

有 MCP client 且 MCP SDK 可用时，AI agent 调用 kdebug 能力应优先走 MCP 托管 session：

1. 用 `kverif_debug_session_open` 打开 design/waveform/combined session。
2. 用 `kverif_debug_query` 传 `action`、`args`、`limits`、`output` 调用本文列出的 kdebug 原生 action。
3. 常规调试不要默认用 `kverif_debug_raw_request`；raw request 是 one-shot CLI 路径，不维护 MCP session manager、stdio-loop 或 LSF job 生命周期。
4. 只有需要完整 kdebug envelope 控制，或验证 raw CLI 行为时，才使用 `kverif_debug_raw_request`。
...
需要 SDK-free wrapper 或 raw CLI 时，再把同一 action 放进本文的原生 JSON request envelope。

本文不是最终契约。最终契约优先级：

1. `kdebug --json -` 调用 `actions`
2. `schema` action 返回的 action-specific schema
3. `kdebug/schemas/v1/actions/*.schema.json`
4. `kdebug/examples/requests/*.basic.json` 与 `kdebug/examples/responses/*.basic.json`
5. 本 reference
...
调用 `actions` 可以在 `data.actions[]` 中读取 `request_schema`、`response_schema`、`request_examples`、`response_examples`。AI agent 需要精确契约时应使用这些 action-specific 文件；通用 `kdebug.request.schema.json` / `kdebug.response.schema.json` 只描述 envelope。

字段说明：

| 字段 | 说明 |
| --- | --- |
| `api_version` | 固定使用 `kdebug.v1` |
...
| 字段 | 位置 | 说明 |
| --- | --- | --- |
| `transport` | `args` 或 `target` | `uds` / `tcp` / `file`，默认 `uds`，可由 `KDEBUG_TRANSPORT` 控制 |
...
`value.batch_at` 部分缺失仍返回 ok；检查 `summary.missing_by_reason` 和 full 输出每个 row 的 `status/reason`。需要 kbit 切字段时给 `value.at` 或 `value.batch_at` 加：

```text
"slice_hint": {"chunk_width": 32, "count": 4}
```

响应里的 `kbit_hints.commands[]` 可直接交给 `tools/kbit`。
...
```bash
tools/kdebug --json - < request.json \
