...
kberif 管理验证项目知识上下文：短 summary cards、detail markdown、brief/context。它不读 RTL/FSDB、不做 bit 计算、不还原日志位置。

## 何时使用

- 初始化 BT/IT/ST/SoC 项目 context。
- 生成或查询 `.kberif/cards.json`、`.kberif/details/*.md`。
- 构造 agent brief/context。
- 检查或修复 card/detail schema、metadata、section 合同。

## 入口

优先 MCP `kverif_context_*`。命令行：
