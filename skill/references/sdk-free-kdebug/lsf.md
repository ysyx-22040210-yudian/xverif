...
```text
bsub -I tools/kdebug --stdio-loop
```

## 常用环境变量

- `KVERIF_LOOP_BACKEND=lsf`
- `KVERIF_LSF_BSUB`：覆盖 bsub 命令。
- `KVERIF_LSF_BKILL`：覆盖 bkill 命令。
- `KVERIF_LSF_SESSION_QUEUE`：session job queue，默认 `interactive`。
- `KVERIF_LSF_SESSION_RESOURCE`：LSF resource string。
- `KVERIF_LOOP_FAKE_LSF=1`：本地 fake LSF 测试。
