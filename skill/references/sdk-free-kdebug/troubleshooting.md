...
默认根目录：`~/.kverif/loop-wrapper`，可用 `KVERIF_LOOP_LOG_DIR` 覆盖。
...
5. 后端 native kdebug session/socket/engine 问题，再读 [../kdebug/troubleshooting.md](../kdebug/troubleshooting.md)。
...
- ready timeout：检查 LSF 队列、backend 是否能启动、`KVERIF_LOOP_STARTUP_TIMEOUT_SEC`。
- query timeout：先缩小 time_range/limits，再考虑增大 `KVERIF_LOOP_REQUEST_TIMEOUT_SEC`。
