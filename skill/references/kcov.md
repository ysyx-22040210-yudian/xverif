...
kcov 查询 VCS/Verdi coverage database（`simv.vdb`、`merged.vdb`）。它负责 coverage evidence，不负责自动解释 hole 根因或生成补测策略。
...
```json
{"api_version":"kcov.v1","action":"session.open","target":{"vdb":"merged.vdb"},"args":{"name":"cov0"}}
```

holes：

```json
{"api_version":"kcov.v1","action":"cov.holes","target":{"session_id":"cov0"},"args":{"metrics":["line","toggle","branch","condition","fsm","assert","functional"],"limits":{"max_items":100}}}
```

source map：

```json
{"api_version":"kcov.v1","action":"source.map","target":{"session_id":"cov0"},"args":{"file":"rtl/ctrl.sv","line":123,"window":3}}
```
...
- action 参数不确定：先用 `kverif_cov_list_actions` 和 `kverif_cov_get_schema`。
