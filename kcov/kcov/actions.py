...
class Dispatcher:
        ...
        try:
            ...
            if action == "actions":
                ...
                if action == "tests.list":
                    ...
                    raise KcovError("ACTION_NOT_FOUND", "unknown action", action=action)
    ...
    def _session(self, req: Json):
        ...
        if not sid:
            raise KcovError("SESSION_NOT_FOUND", "target.session_id is required")
        return self.sessions.get(str(sid))

    def _actions(self, req: Json) -> Json:
        rows = [{"name": a, "status": "p0", "api_version": "kcov.v1"} for a in P0_ACTIONS]
    ...
    def _schema(self, req: Json) -> Json:
        ...
        if action not in P0_ACTIONS and action not in ("actions", "schema"):
            raise KcovError("ACTION_NOT_FOUND", "schema action not found", action=action)
        kind = str(merged_action_args(req).get("kind", "request"))
        try:
            schema = schema_for_action(str(action), kind)
        except KeyError:
            raise KcovError("ACTION_NOT_FOUND", "schema action not found", action=action, kind=kind)
    ...
    def _session_open(self, req: Json) -> Json:
        ...
        vdb = target.get("vdb")
        if not vdb:
            raise KcovError("VDB_OPEN_FAILED", "target.vdb is required")
    ...
    def _source_map(self, req: Json, sess) -> Json:
        ...
        if file_name is None or line is None:
            raise KcovError("SCHEMA_INVALID", "source.map requires file and line")
        metrics = args.get("metrics")
    ...
    def _export(self, req: Json, sess) -> Json:
        ...
        if action == "export.summary":
            ...
            raise KcovError("ACTION_NOT_FOUND", "unknown export action", action=action)
        query = query_args(args)
        rows = filter_items(rows, query)
        rows = sort_items(rows, args.get("sort"))
...
def _object_get_rows(sess: Any, args: Json, metrics: Any) -> List[Json]:
    ...
    if not rows:
        raise KcovError("OBJECT_NOT_FOUND", "coverage object not found", name=name)
    return rows
