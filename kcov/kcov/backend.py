...
class FakeCoverageBackend(CoverageBackend):
    ...
    def _check_test(self, test: str) -> None:
        if test == "each":
            raise KcovError("TEST_MODE_NOT_SUPPORTED",
...
class NpiCoverageBackend(CoverageBackend):
    ...
    def __post_init__(self) -> None:
        raw_timeout = os.environ.get("KVERIF_KCOV_TCL_TIMEOUT_SEC")
    ...
    def _check_test(self, test: str) -> None:
        ...
        if any(row.get("name") == test for row in self.tests()):
            return
        raise KcovError("TEST_NOT_FOUND", "test not found", test=test)
    ...
    def _run_tcl(self, action: str, args: Optional[Json] = None) -> Json:
        ...
        if not verdi:
            ...
            raise KcovError("VERDI_NOT_FOUND", "verdi is not in PATH; set VERDI_HOME")
        script = _tcl_script_path()
        request = {"action": action, "vdb": self.vdb, "args": args or {}}
        tmpdir = tempfile.mkdtemp(prefix="kcov-tcl-npi-")
        try:
            ...
            env = dict(os.environ)
            env["KCOV_TCL_REQUEST_JSON"] = req_path
            env["KCOV_TCL_RESPONSE_JSON"] = rsp_path
            env["KCOV_TCL_ACTION"] = action
            env["KCOV_TCL_VDB"] = self.vdb
            action_args = args or {}
            metrics = action_args.get("metrics") if isinstance(action_args.get("metrics"), list) else []
            env["KCOV_TCL_METRICS"] = "\n".join(str(metric) for metric in metrics)
            env["KCOV_TCL_SCOPE"] = "" if action_args.get("scope") is None else str(action_args.get("scope"))
            env["KCOV_TCL_TEST"] = str(action_args.get("test", "merged"))
            env["KCOV_TCL_FUNCTIONAL_ONLY"] = "1" if action_args.get("functional_only") else "0"
            if os.environ.get("KVERIF_KCOV_VERDI_HOME"):
                env["VERDI_HOME"] = os.environ["KVERIF_KCOV_VERDI_HOME"]
            ...
            try:
                ...
                log_lifecycle_event("adhoc", "npi.tcl.invoke.timeout", False,
                                    {"vdb": self.vdb, "action": action})
                raise KcovError("TCL_NPI_TIMEOUT", "Verdi Tcl coverage action timed out",
                ...
                raise KcovError("VERDI_EXEC_FAILED", str(exc)) from exc
            try:
                ...
                raise KcovError("TCL_NPI_NO_RESPONSE",
            ...
            if not payload.get("ok"):
                ...
                raise KcovError(str(code), str(message), tcl_action=action,
...
def _find_verdi() -> Optional[str]:
    verdi_home = os.environ.get("KVERIF_KCOV_VERDI_HOME") or os.environ.get("VERDI_HOME")
    if verdi_home:
...
def _tcl_script_path() -> Path:
    return Path(__file__).resolve().parents[1] / "tcl_engine" / "kcov_npi.tcl"
...
def _branch_mask_hint_enabled() -> bool:
    return str(os.environ.get("KVERIF_KCOV_BRANCH_MASK_HINT", "1")).lower() not in {
        "0", "false", "no", "off",
    }
