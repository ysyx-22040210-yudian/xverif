...
> 2026-07-23 审计更正：历史 `with_xdebug` 组没有在 repair trial 中执行 KDebug。
> 其 evidence 是已有 `fail/run.log` 的复制或裁剪，不是 FSDB/KDB 动态查询结果。本文保留
> 两组模型轨迹，但不再把组间差异归因于 KDebug。完整证据见
> [工具调用与逐 Case 作用审计](benchmark_kdebug_evidence_audit.md)。
...
| 本文名称 | 历史目录 | 含义 |
| --- | --- | --- |
| 名义 KDebug 组 | `with_xdebug` | runner 额外拼入标为 xdebug 的预置文本；后续审计确认这些文本重复 shared fail log，未真实调用工具 |
...
flowchart LR
    ...
    K["预置 evidence 文本，仅名义 KDebug 组"] --> B
...
- `evidence_used=true` 表示该 trial 属于名义 KDebug 组，不证明执行过 KDebug，也不证明
  模型在语义上正确利用了 evidence。
...
| Case | 真实注错摘要 | GPT 名义工具组 | GPT 基线 | Qwen 名义工具组 | Qwen 基线 | GLM 名义工具组 | GLM 基线 |
| --- | --- | --- | --- | --- | --- | --- | --- |
...
名义工具组的 Qwen 把失败样本的共同 offset 与 RTL 条件关联起来，但这些地址和值也在
基线组共享的 `fail/run.log` 中。该单侧 PASS 只能说明两次模型采样和补丁落地不同，不能
归因于 KDebug 动态窗口。
...
- 名义 KDebug 组根据共享失败日志中的寄存器差异和后续请求到的 RTL，把范围缩小到
  `Alu.sv/Alu_3.sv`，第二轮看到注入表达式后把输出恢复为 `_T_0`。完整 full-chip
  仿真耗时约 2644 秒，最终 PASS。审计确认所谓 `xdebug_commit_window.json` 只是同一
  `fail/run.log` 的尾部，不含独立的 KDebug first-bad commit 查询。
...
这是名义工具组与基线组结果差异最大的 case，但历史材料不足以证明差异来自 KDebug。
可以确认 GPT 找到了正确实例并闭环 PASS；不能确认有动态工具调用帮助它找到首个坏值。
看到最后报错 PC 也不等于该 PC 的指令写坏了寄存器。
...
- 名义 KDebug 组根据共享失败日志中的 commit/exception 信息发现执行从 `0x800001ea`
  redirect 到错误的 `0x800001dc`，而正确 loop body 在 `0x800001de`，请求
  `BranchUnit.sv` 并删除 XOR 2，第 2 轮 PASS。
...
这是 metadata anchoring 的典型反例。公开 subsystem 标签错误，而名为 TLB/cache trace
的 evidence 实际仍是 case 006 的 ALU 失败日志，无法纠正标签。历史结果不能把该 case
当成 cache/MMU 调试能力测试。
...
- 名义 KDebug 组指出 loop branch 落到错误 RVC halfword，并从可见配置发现 Difftest 参数
  为空；修正 `BranchUnit.sv`、`config/diff.env` 和 run 参数，第 3 轮 PASS。所谓
  `xdebug_ref_so_audit.md` 实际只复制基础 case 009 的 run log，没有执行命令审计。
...
- `case_006` 的名义工具组最终落到 `Alu_3.sv` 而基线组没有，但 evidence 审计表明两组
  获得的有效失败信息相同，不能把这次组间差异归因于 KDebug。
...
- full-chip case 中多次错误解码指令和寄存器，导致从 load/branch 漂移到 AMO、BPU、DMA
  或 bypass 模块。
- 名义工具组的总 PASS 数较高，但新增文本是 shared log 的重复包装，不能据此宣称 KDebug
  提高了成功率。Qwen 仍需要可靠的指令解码、last-writer 和 signal provenance。
...
### 6.4 KDebug evidence 审计后的真实增量

2026-07-23 对 16 个 case 的 evidence、runner 和 matrix log 重新审计后确认：

- repair trial 内没有执行 KDebug、Verdi、NPI 或 FSDB action；
- 所有 JSON/Markdown 的主要 payload 都是两组共享 `fail/run.log` 的精确子串；
- `001-005` 的 CSV 是同一份通用模板；
- `014-016` 的 evidence 从基础 case 继承，没有覆盖新增环境注错；
- `evidence_used` 和 `tool_evidence_present` 只表示文件存在和分组，不是调用证明。

因此本节不能再列出“工具真正帮助的 case”。GPT `case_006`、Qwen `case_002/005` 的单侧
PASS 是真实结果，但因缺少独有工具观测和调用 provenance，只能归为 prompt/模型采样与
补丁落地差异。逐 case 证据见
[KDebug 工具调用与作用审计](benchmark_kdebug_evidence_audit.md)。
...
GPT-5.5 在静态异常、环境问题和部分 full-chip case 上闭环最好；Qwen
在简单 case 上速度快，但容易陷入重复诊断和无效 patch；GLM 因未完成补测和 API 限流，
只能如实报告已有 `001-007` 轨迹。历史 suite 没有真实执行 KDebug，不能用名义工具组较高
的 PASS 数证明工具提升；模型能力结论只覆盖读取现有日志、静态文件、迭代反馈和完成回归
验证的表现。
