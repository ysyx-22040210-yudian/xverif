...
`with_kdebug` 额外获得预先采集的动态证据，例如：

- AXI handshake、beat、status 和 first mismatch 窗口。
- commit/writeback、rfWen、redirect、load data 等 first divergence。
- 实际 simv 命令、Difftest enable、case dispatch、simv fingerprint 和 timeout 审计。

`without_kdebug` 不得使用 FSDB、KDB、Verdi/NPI 或这些动态证据，只能依据日志、
静态 RTL、脚本配置、公开设计约束和反复 build/run/judge 的结果定位。
