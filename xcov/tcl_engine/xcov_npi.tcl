...
proc write_response_raw {json_text} {
    if {![info exists ::env(XCOV_TCL_RESPONSE_JSON)] || $::env(XCOV_TCL_RESPONSE_JSON) eq ""} {
        puts $json_text
        return
    }
    set fp [open $::env(XCOV_TCL_RESPONSE_JSON) w]
    puts $fp $json_text
    close $fp
}
...
proc metric_list {} {
    set raw [env_or_empty XCOV_TCL_METRICS]
    set out {}
...
proc open_db {} {
    set vdb [env_or_empty XCOV_TCL_VDB]
    if {$vdb eq ""} {
        fail_data "VDB_OPEN_FAILED" "XCOV_TCL_VDB is required"
        return ""
    }
...
proc selected_test {db tests} {
    set wanted [env_or_empty XCOV_TCL_TEST]
...
proc items_action {db tests} {
    ...
    set scope_filter [env_or_empty XCOV_TCL_SCOPE]
    set functional_only [env_flag XCOV_TCL_FUNCTIONAL_ONLY]
    foreach inst [cov_iter_children $db npiCovInstance] {
...
proc main {} {
    source_l1
    set action [env_or_empty XCOV_TCL_ACTION]
    set db [open_db]
    if {$db eq ""} {return}
