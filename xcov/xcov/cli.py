...
def _apply_key_value(root: Json, item: str) -> None:
    if "=" not in item or item.startswith("="):
        raise XcovError("INVALID_CLI", f"expected key=value, got: {item}")
...
def build_shortcut_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="xcov", description="VCS/Verdi coverage query tool")
    sub = parser.add_subparsers(dest="command", required=True)

    p = sub.add_parser("actions", help="list xcov actions")
    ...
    p = sub.add_parser("query", help="run any xcov action without writing JSON")
...
def request_from_shortcut(ns: argparse.Namespace, force_json: bool = False) -> Json:
    ...
    req: Json = {"api_version": "xcov.v1", "request_id": f"cli-{ns.command}", "action": action}
...
def run_shortcut(argv: List[str], dispatcher: Dispatcher, force_json: bool = False) -> int:
    ...
    try:
        ...
        req = {"api_version": "xcov.v1", "request_id": "cli-error", "action": getattr(ns, "command", "")}
        if force_json or getattr(ns, "json", False):
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    while shortcut_argv and shortcut_argv[0] in {"--json", "--text", "--xout"}:
