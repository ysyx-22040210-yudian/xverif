...
只运行工具组时必须显式使用 `--groups with_kdebug`。需要对照实验时才使用默认的
`with_kdebug,without_kdebug`；日志中的 `MATRIX CONFIG` 行是最终模型和组选择的审计依据。
例如跳过 GLM、只运行两个非 GLM 模型和工具组：
...
- 工具组每个拟应用补丁都必须在公开回复中给出
  `KDEBUG_EVIDENCE_USED: <plan 声明文件> | <具体信号/driver/时间/命令事实>`；文件未声明、
  事实为空泛或缺少该行时，runner 不应用补丁。
