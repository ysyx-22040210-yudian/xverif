...
正常执行时终端和 `~/.keda_runner/<pid>.log` 同步输出：

```
[keda-runner] log: /home/yian/.keda_runner/543212.log
[keda-runner] runner_pid=543212 runner_pgid=543212
[keda-runner] child_pid=543213 child_pgid=543212
[keda-runner] action=sim target=compile
[keda-runner] command: make -j8 compile TEST=smoke_test SEED=123
[keda-runner] cwd=/proj/nic/work
... (命令 stdout/stderr) ...
[keda-runner] exit_code=0
```

- `runner_pid`：keda-runner 自身进程
...
`.keda-runner.yaml`，详细格式见目录下示例文件。主要字段：
...
keda-runner 是阻塞式 runner，不会后台化执行。如果预估命令执行时间超过 **5 分钟**，建议使用 `tmux` 或 `nohup` 保证任务不会被 terminal / shell 生命周期影响：

```bash
# tmux（推荐）
tmux new -d -s sim "keda-runner run --action sim --target compile --option TEST=smoke"

# 查看输出
tmux capture-pane -t sim -p

# nohup
nohup keda-runner run --action sim --target compile --option TEST=smoke > sim.log 2>&1 &

# 查看输出
tail -f sim.log
```

keda-runner 只负责启动进程，不负责后台存活和结果监控。
