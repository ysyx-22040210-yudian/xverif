#!/usr/bin/env python3
"""keda-runner — 带环境快照缓存的阻塞式 allowlist command runner.
...
def find_config() -> str:
    ...
    path = os.path.join(os.getcwd(), ".keda-runner.yaml")
    if os.path.isfile(path):
        return path
    return ".keda-runner.yaml"
...
_RUNNER_LOG_DIR = Path.home() / ".keda_runner"
...
def _init_runner_log() -> TextIO:
    ...
    print(f"[keda-runner] log: {log_path}", file=sys.stderr)
    return _RUNNER_LOG
...
def generate_init_script(cfg: dict[str, Any]) -> str:
    ...
    if checks:
        lines.append("")
        lines.append('echo "[keda-runner] checks"')
        for tool in checks:
    ...
    timestamp_file = Path(workdir) / ".keda_runner" / "timestamp"
...
def cmd_init(args: argparse.Namespace) -> int:
    ...
    if snapshot.exists() and not args.refresh:
        print(f"[keda-runner] env snapshot already exists: {snapshot}",
              file=sys.stderr)
        print("[keda-runner] use --refresh to re-initialize", file=sys.stderr)
        return 0
    ...
    try:
        with open(init_log, "w") as log:
            log.write(f"# keda-runner init log — {time.strftime('%Y-%m-%d %H:%M:%S')}\n")
        ...
        if result.returncode != 0:
            print(f"[keda-runner] init failed (exit {result.returncode})",
                  file=sys.stderr)
            print(f"[keda-runner] see log: {init_log}", file=sys.stderr)
        ...
        print(f"[keda-runner] init complete — {snapshot}", file=sys.stderr)
        return 0
    finally:
...
def cmd_describe_action(args: argparse.Namespace) -> int:
    ...
    if args.action not in actions:
        print(f"[keda-runner] action not found: {args.action}", file=sys.stderr)
        return 2
...
def cmd_run(args: argparse.Namespace) -> int:
    ...
    actions = cfg.get("actions", {})
    if args.action not in actions:
        print(f"[keda-runner] action not found: {args.action}",
              file=sys.stderr)
        return 2
    ...
    if args.dry_run:
        _runner_print(f"[keda-runner] DRY-RUN runner_pid={os.getpid()} "
                      f"runner_pgid={os.getpgid(0)}")
        _runner_print(f"[keda-runner] DRY-RUN command: {' '.join(argv)}")
        _runner_print(f"[keda-runner] DRY-RUN cwd={workdir}")
        return 0

    # real run: snapshot required
    snapshot = env0_path(cfg, workdir)
    if not snapshot.exists():
        _runner_print(f"[keda-runner] env snapshot not found: {snapshot}")
        _runner_print(f"[keda-runner] run: keda-runner init")
        return 2
    ...
    if not args.quiet:
        _runner_print(f"[keda-runner] runner_pid={os.getpid()} "
                      f"runner_pgid={os.getpgid(0)}")
        _runner_print(f"[keda-runner] child_pid={proc.pid} "
                      f"child_pgid={os.getpgid(proc.pid)}")
        _runner_print(f"[keda-runner] action={args.action} "
                      f"target={args.target or ''}")
        _runner_print(f"[keda-runner] command: {' '.join(argv)}")
        _runner_print(f"[keda-runner] cwd={workdir}")

    proc.wait()
    _runner_print(f"[keda-runner] exit_code={proc.returncode}")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"[keda-runner] error: {e}", file=sys.stderr)
        return 2
