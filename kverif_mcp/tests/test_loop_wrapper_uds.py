...
from kverif_loop.config import configure_mcp_environment
from kverif_loop.logging import configure_mcp_logging
from kverif_loop.wrapper import (
    LoopWrapperServer,
    LoopWrapperService,
...
for line in sys.stdin:
    ...
    if wants_json:
        ...
        rsp = {{"id": rid, "ok": True, "payload_format": "kout", "kout": "@{api_version}." + action + ".v1\\n"}}
...
def test_loop_wrapper_uses_kcov_unlimited_defaults(tmp_path, monkeypatch):
    monkeypatch.setenv("KVERIF_LOOP_STARTUP_TIMEOUT_SEC", "181")
    monkeypatch.setenv("KVERIF_LOOP_REQUEST_TIMEOUT_SEC", "361")
    monkeypatch.delenv("KVERIF_KCOV_STARTUP_TIMEOUT_SEC", raising=False)
    monkeypatch.delenv("KVERIF_KCOV_REQUEST_TIMEOUT_SEC", raising=False)
    service = LoopWrapperService(mode="direct", kdebug_bin="kdebug", kcov_bin="kcov")
...
def test_loop_wrapper_explicit_timeouts_apply_to_both_backends():
    service = LoopWrapperService(
        mode="direct", kdebug_bin="kdebug", kcov_bin="kcov",
...
def test_loop_wrapper_ping(tmp_path, monkeypatch):
    monkeypatch.setenv("KVERIF_LOOP_LOG_DIR", str(tmp_path / "logs"))
    debug = _make_fake_loop(tmp_path / "fake_kdebug", protocol="kdebug-stdio-loop", api_version="kdebug")
    cov = _make_fake_loop(tmp_path / "fake_kcov", protocol="kcov-stdio-loop", api_version="kcov")
    service = LoopWrapperService(mode="direct", kdebug_bin=debug, kcov_bin=cov)
...
def test_loop_wrapper_cov_query_waits_with_zero_timeout(tmp_path, monkeypatch):
    monkeypatch.setenv("KVERIF_LOOP_LOG_DIR", str(tmp_path / "logs"))
    debug = _make_fake_loop(tmp_path / "fake_kdebug", protocol="kdebug-stdio-loop",
                            api_version="kdebug")
    cov = _make_fake_loop(tmp_path / "fake_kcov", protocol="kcov-stdio-loop",
                          api_version="kcov", query_delay=0.15)
    service = LoopWrapperService(mode="direct", kdebug_bin=debug, kcov_bin=cov)
    server, thread, sock = _start_server(tmp_path, service)
    try:
        responses = send_requests(sock, [
...
def test_loop_wrapper_writes_direct_structured_logs(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("KVERIF_LOOP_LOG_DIR", str(log_root))
    debug = _make_fake_loop(tmp_path / "fake_kdebug", protocol="kdebug-stdio-loop", api_version="kdebug")
    cov = _make_fake_loop(tmp_path / "fake_kcov", protocol="kcov-stdio-loop", api_version="kcov")
    service = LoopWrapperService(mode="direct", kdebug_bin=debug, kcov_bin=cov)
    server, thread, sock = _start_server(tmp_path, service)
    ...
    assert {e["component"] for e in uds_events} == {"kverif-loop-wrapper"}
...
def test_loop_wrapper_fake_lsf_logs_job_and_cleanup(tmp_path, monkeypatch):
    log_root = tmp_path / "logs"
    monkeypatch.setenv("KVERIF_LOOP_LOG_DIR", str(log_root))
    monkeypatch.setenv("KVERIF_LOOP_FAKE_LSF", "1")
