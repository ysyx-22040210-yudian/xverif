...
from kverif_loop.logging import argv_hash, log_lsf_event
...
class BsubRunner:
    def __init__(self, bsub_cmd: Optional[str] = None) -> None:
        self.bsub_cmd = bsub_cmd or os.environ.get("KVERIF_LSF_BSUB", "bsub")
    ...
    def build(self, command: Iterable[str], opts: Optional[BsubOptions] = None) -> List[str]:
        ...
        # KVERIF_LSF_BSUB, but we default to -I for cleaner machine-protocol output.
        interactive = {"-I", "-Is", "-Ip"}
