...
from kverif_loop.lsf.bsub import BsubOptions, BsubRunner
from kverif_loop.lsf.protocol import JsonlProcess

from kverif_loop.config import bkill_timeout, fake_lsf_enabled
...
def _bkill_by_id(job_id: str) -> None:
    bkill_cmd = os.environ.get("KVERIF_LSF_BKILL", "bkill")
    try:
...
class DirectLauncher(Launcher):
    ...
    def start(self, cfg: LaunchConfig) -> JsonlProcess:
        cmd = [cfg.tool_bin or cfg.kdebug_bin, "--stdio-loop"]
