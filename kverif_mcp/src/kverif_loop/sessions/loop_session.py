...
from kverif_loop.logging import log_session_event

from kverif_loop.config import default_kdebug_bin, startup_timeout, request_timeout, close_timeout
from kverif_loop.sessions.launchers import LaunchConfig, Launcher
from kverif_loop.sessions.session_errors import response_says_session_terminal
...
@dataclass
class KdebugLoopSession:
    alias: str
    fsdb: Optional[str]
    daidir: Optional[str]
    launcher: Launcher
    kdebug_bin: str = field(default_factory=default_kdebug_bin)
    backend: str = "kdebug"
    api_version: str = "kdebug.v1"
    ready_protocol: str = "kdebug-stdio-loop"
    target_key: str = "fsdb"
    recovery_tool: str = "kverif_debug_session_open"
    ...
    def _session_lost_error(self, message: str, *, source: str,
        ...
        if source == "transport" and "timeout" in message.lower():
            reason = (
                ...
                "KVERIF_MCP_STARTUP_TIMEOUT_SEC (session open, default 180s) "
                "or KVERIF_MCP_REQUEST_TIMEOUT_SEC (query, default 360s). "
    ...
    def open(self) -> Json:
        ...
        cfg = LaunchConfig(alias=self.alias, kdebug_bin=self.kdebug_bin,
                           backend=self.backend,
                           tool_bin=self.kdebug_bin,
                           queue=self.queue, resource=self.resource,
                           job_name=self.job_name,
        ...
        try:
            ...
            open_req: Json = {
                ...
                "output": {"response_format": "json"} if self.backend == "kcov"
                else {"format": "json"},
            }
            open_req["trace_id"] = _trace_id(self.alias, open_req["request_id"])
            if self.backend == "kdebug":
        ...
        if self.handle and self.state == "alive" and self.session_id and not force:
            ...
            try:
                req = {
                    ...
                    "api_version": "kdebug.v1", "action": "stdio.quit",
                }
    ...
    def query(self, action: str, args: Optional[Json] = None,
              ...
              output: Optional[Json] = None, output_format: str = "kout") -> Any:
