"""Terminal session error detection for kdebug loop sessions.
...
def error_code(rsp: Json) -> str:
    """Extract the ``error.code`` from a (possibly nested) kdebug response."""
    err = rsp.get("error")
...
def response_says_session_terminal(rsp: Json) -> bool:
    ...
    Scans the top-level envelope, ``rsp["json"]`` (the nested kdebug reply
