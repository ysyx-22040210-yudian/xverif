"""SDK-free UDS JSONL wrapper for kverif stateful loop sessions."""
...
from kverif_loop.config import (
    ...
    startup_timeout,
    request_timeout,
    kcov_startup_timeout,
    kcov_request_timeout,
)
from kverif_loop.logging import (
...
class LoopWrapperService:
    def __init__(
        ...
        *,
        ...
        cov_startup_timeout = kcov_startup_timeout() if startup_timeout_sec is None else startup_timeout_sec
        cov_request_timeout = kcov_request_timeout() if request_timeout_sec is None else request_timeout_sec
        self.mode = mode or loop_backend()
        self.debug = McpSessionManager(
            mode=self.mode,
            kdebug_bin=kdebug_bin or default_kdebug_bin(),
            startup_timeout_sec=debug_startup_timeout,
            request_timeout_sec=debug_request_timeout,
            backend="kdebug",
            api_version="kdebug.v1",
            ready_protocol="kdebug-stdio-loop",
            target_key="fsdb",
            recovery_tool="debug.session.open",
        )
        self.cov = McpSessionManager(
            mode=self.mode,
            kdebug_bin=kcov_bin or default_kcov_bin(),
    ...
    def _dispatch_method(self, method: str, params: Json) -> Any:
        ...
        if method == "debug.query":
            return self.debug.query(
                ...
                output_format=params.get("output_format", "kout"),
            )
...
def _add_query_options(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument("--output-format", choices=("kout", "json", "envelope"), default="kout")
...
def build_client_shortcut_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="kverif-loop-client",
        description="Send parameter-style requests to kverif-loop-server",
    )
    ...
    p = sub.add_parser("debug-open", help="open a kdebug loop session")
    ...
    sub.add_parser("debug-list", help="list kdebug loop sessions")

    p = sub.add_parser("debug-close", help="close a kdebug loop session")
    p.add_argument("--session", "--session-id", "--name", dest="session", required=True)

    p = sub.add_parser("debug-query", help="query a kdebug loop session")
    _add_query_options(p)

    p = sub.add_parser("cov-open", help="open a kcov loop session")
    p.add_argument("--name", required=True)
    p.add_argument("--vdb", required=True)
    p.add_argument("--queue")
    p.add_argument("--resource")

    sub.add_parser("cov-list", help="list kcov loop sessions")

    p = sub.add_parser("cov-close", help="close a kcov loop session")
    p.add_argument("--session", "--session-id", "--name", dest="session", required=True)

    p = sub.add_parser("cov-query", help="query a kcov loop session")
    _add_query_options(p)
    return parser
...
def server_main(argv: Optional[List[str]] = None) -> int:
    parser = argparse.ArgumentParser(description="Run the kverif SDK-free loop wrapper server")
    parser.add_argument("--socket", default=default_socket_path())
    parser.add_argument("--backend", choices=("direct", "lsf"), default=None)
    args = parser.parse_args(argv)
    if args.backend:
        os.environ["KVERIF_LOOP_BACKEND"] = args.backend
...
def client_main(argv: Optional[List[str]] = None) -> int:
    ...
    if not argv or argv[0] in {"-h", "--help"}:
        ...
        parser = argparse.ArgumentParser(prog="kverif-loop-client")
        parser.add_argument("--socket", default=default_socket_path())
    ...
    if shortcut_argv and shortcut_argv[0] in shortcut_commands:
        ...
        parser = argparse.ArgumentParser(description="Send requests to kverif-loop-server")
        parser.add_argument("--socket", default=default_socket_path())
        parser.add_argument("--json", help="single JSON request object")
        parser.add_argument("--timeout-sec", type=float, default=None,
                            help="default: unlimited for cov.*, 30 for other methods; 0 disables")
