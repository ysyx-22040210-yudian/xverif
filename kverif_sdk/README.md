...
`kverif_sdk` 是给验证工程师做二次开发的公共 Python 层。它不替代
`kdebug`、`kcov`，也不直接调用 NPI；它把工具现有的 JSON/stdio-loop
...
- 在公司内部 RPC、LSF wrapper 或测试框架中复用 kverif，而不修改工具核心。

SDK 只使用 Python 标准库，支持 Python 3.8+。

## 公共 API 与兼容性约定

- 只把 `kverif_sdk.__init__` 导出的对象视为公共 Python API；以下划线开头的
...
| client | `KdebugClient` | session、`value.batch_at`、`signal.changes`、`trace.driver/graph` |
| client | `KcovClient` | coverage session、summary、holes |
...
- 导入 `kdebug/src`、`kcov/kcov/backend.py` 或 `kverif_mcp` 内部 session 类。
- 解析给人看的 kout 或 stderr 文本。
...
`tools/kcov` 子进程，wrapper 通过 `PYTHON` 环境变量选择解释器；若未导出，旧 VM
...
```python
from kverif_sdk import StdioTransport, KcovClient, resolve_tool
...
```bash
/home/host/kverif/.venv38/bin/python -m kverif_sdk.examples.waveform_analysis \
...
```python
from kverif_sdk import analyze_wave_window
...
输出 `schema` 为 `kverif.sdk.wave-window.v1`。每个信号保留完整
...
```bash
/home/host/kverif/.venv38/bin/python -m kverif_sdk.examples.module_connectivity \
  --tool /home/host/kverif/tools/kdebug \
...
`kverif.sdk.module-connections.v1`，并包含：
...
```bash
/home/host/kverif/.venv38/bin/python -m kverif_sdk.examples.coverage_convergence \
...
```python
from kverif_sdk import CoverageRun, analyze_coverage_convergence
...
输出 `schema` 为 `kverif.sdk.coverage-convergence.v1`：
...
class InternalRpcTransport:
    ...
    def request(self, request, timeout_sec=None):
        return self.rpc.call("kverif", request, timeout=timeout_sec)

debug = KdebugClient(InternalRpcTransport(company_rpc))
```
...
debug = KdebugClient(CallbackTransport(fake_tool))
```
...
```bash
cd /home/host/kverif
export PYTHONPATH=/home/host/kverif
/home/host/kverif/.venv38/bin/python -m pytest kverif_sdk/tests -q
```
...
- kdebug/kcov session 自动 open/close 与参数映射。
