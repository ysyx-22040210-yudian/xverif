...
from kverif_sdk import (CallbackTransport, CliTransport, ProtocolError,
...
def test_cli_transport_timeout_and_json(monkeypatch, configured, expected):
    ...
    monkeypatch.setattr("kverif_sdk.transport.subprocess.run", fake_run)
    transport = CliTransport("kdebug", timeout_sec=configured)
    response = transport.request({"api_version": "kdebug.v1", "action": "actions"})
    assert response["ok"] is True
    assert observed["command"] == ["kdebug", "--json", "-"]
...
print(json.dumps({"type":"ready","protocol":"kdebug-stdio-loop","version":1,"pid":os.getpid()}), flush=True)
for line in sys.stdin:
    ...
    if action == "fail":
        ...
        response = {"ok":True,"api_version":"kdebug.v1","action":action,"data":{"request":request}}
...
def test_stdio_transport_delayed_request_and_structured_error(tmp_path):
    ...
    transport = StdioTransport(
        command, protocol="kdebug-stdio-loop", api_version="kdebug.v1",
        startup_timeout_sec=2, request_timeout_sec=0,
    )
    with transport:
        response = transport.request({
            "api_version": "kdebug.v1", "action": "value.at",
            "args": {"sleep": 0.05},
        })
        assert response["ok"] is True
        assert response["data"]["request"]["request_id"].startswith("sdk-")
        failed = transport.request({"api_version": "kdebug.v1", "action": "fail"})
