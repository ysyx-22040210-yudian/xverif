...
当 xdebug 响应里出现 `xbit_hints.commands[]` 时，直接执行其中的 `tools/xbit ...` 命令；不要把 packed value 的 slice、signed 或字段比较留给自然语言推理。默认输出是 `xout`，需要脚本解析字段时才加 `--json`。
...
如果当前 shell 尚未安装 `xbit`，并且当前工作目录就是仓库根目录，可以临时使用 `tools/xbit conv "8'shff"`。
