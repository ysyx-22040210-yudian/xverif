...
`transport/host/bind_host/port/socket_path` 通常只在 `session.open/list/doctor`、`output.verbosity:"full"` / `"debug"` 或后端日志中出现。compact action 响应不会反复携带 endpoint 信息；连接问题优先读 `~/.xdebug/{design,waveform}/sessions/<hashed-session>/logs/transport.ndjson`。
