...
int run_query(const Json& req, long long elapsed_ms) {
    ...
    auto log_return = [&](const Json& out, int rc) -> int {
        xdebug_core::log_action_event("backend", "waveform", log_sid, action, "end",
            out.value("ok", rc == 0), elapsed_ms,
            {{"response", xdebug_core::response_summary_for_log(out)},
             {"request", out.value("ok", false) ? xdebug_core::request_summary_for_log(req)
                                                : xdebug_core::sanitize_for_log(req)}});
