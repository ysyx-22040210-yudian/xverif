...
Action 协议由 `ActionSpec` / `ActionRegistry` 约束。`actions` 输出来自 runtime registry，并带有 `category`、`status`、`requires`、schema 和 example 信息；`xdebug/specs/actions/actions.yaml`、`xdebug/schemas/v1` 与 `xdebug/examples` 由 contract test 校验一致。
